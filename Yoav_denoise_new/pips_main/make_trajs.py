...
from pip_utils.basic import readPFM
import random
import glob
...
def helper(rgb_path, mask_path, flow_path, out_dir, folder_name, lr, start_ind, sw=None, include_vis=False):
    ...
    coords.append(coord)
    for s in range(S-1):
        delta = pip_utils.samp.bilinear_sample2d(all_flows_f[:,s], coord[:,:,0].round(), coord[:,:,1].round()).permute(0,2,1) # B,N,2: forward flow at the discrete points
        coord = coord + delta
        coords.append(coord)
