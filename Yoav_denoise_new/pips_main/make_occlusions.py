...
from pip_utils.basic import readPFM, print_stats
import random
import glob
...
def consider_id(id_, all_rgbs, all_masks, all_flows_f, all_flows_b,
    ...
    for s in range(S):
        ...
        if torch.sum(single) > 4:
            ...
            hu_match_amount = torch.tensor(1.0, device=DEVICE)
    ...
    for s in range(S-1):
        ...
        if torch.sum(single) > min_size:
            ys, xs = pip_utils.basic.meshgrid2d(1, H, W)
            ...
            if len(xs_) > min_size:
                ...
                fw_match_amount = torch.tensor(1.0, device=DEVICE)
        else:
            fw_match_amount = torch.tensor(1.0, device=DEVICE)
        fw_match_amounts.append(fw_match_amount)
    fw_match_amounts.append(torch.tensor(1.0, device=DEVICE))
    
    bw_match_amounts = []
    bw_match_amounts.append(torch.tensor(1.0, device=DEVICE))
    for s in range(S-1):
        single = singles[:,s]
        single_next = singles[:,s+1]
        if torch.sum(single) > min_size:
            ...
            if len(xs_) > min_size:
                ...
                bw_match_amount = torch.tensor(1.0, device=DEVICE)
        else:
            bw_match_amount = torch.tensor(1.0, device=DEVICE)
    ...
    if len(xs):
        ...
        coords.append(coord)
        for s in range(S-1):
            # delta = pip_utils.samp.bilinear_sample2d(all_flows_f[:,s], coord[:,:,0].round(), coord[:,:,1].round()).permute(0,2,1) # 1,N,2: forward flow at the discrete points
        ...
        trajs_e = torch.zeros((1,S,0,2), dtype=torch.float32, device=DEVICE)
...
def helper(rgb_path, mask_path, flow_path, out_dir, folder_name, lr, start_ind, sw=None, include_vis=False):
    ...
    for img_name in img_names:
        ...
        try:
            ...
            sys.stdout.flush()
            return
    bak_all_rgbs = pip_utils.improc.preprocess_color(torch.from_numpy(np.stack(rgbs, 0)).to(device)).permute(0,3,1,2).unsqueeze(0)
        ...
        if singles is not None:
            ...
            if include_vis:
                ...
                if trajs.shape[2] > max_show:
                    inds = pip_utils.geom.farthest_point_sample(trajs[:,0], max_show, deterministic=False)
            ...
            for folder_name in folder_names:
                for lr in ['left', 'right']:
                    for start_ind in [0,1,2]:
                        global_step += 1
                        sw = pip_utils.improc.Summ_writer(
                            writer=writer,
