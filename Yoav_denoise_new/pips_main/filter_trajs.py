...
def filter_trajs(trajs_e, all_masks, all_flows_f, all_flows_b):
    ...
    id0 = pip_utils.samp.bilinear_sample2d(all_masks[:,0], trajs_e[:,0,:,0].round(), trajs_e[:,0,:,1].round()).reshape(-1) # N
    ...
    for s in range(S):
        ...
        for dx in [-1,0,1]:
            for dy in [-1,0,1]:
                idi = pip_utils.samp.bilinear_sample2d(all_masks[:,s], trajs_e[:,s,:,0].round() + dx, trajs_e[:,s,:,1].round() + dy).reshape(-1) # N
                id_ok = id_ok & (idi == id0)
    ...
    for s in range(S-1):
        ff = pip_utils.samp.bilinear_sample2d(all_flows_f[:,s], trajs_e[:,s,:,0].round(), trajs_e[:,s,:,1].round()).permute(0,2,1).reshape(-1, 2) # N,2
        bf = pip_utils.samp.bilinear_sample2d(all_flows_b[:,s], trajs_e[:,s+1,:,0].round(), trajs_e[:,s+1,:,1].round()).permute(0,2,1).reshape(-1, 2) # N,2
