import sys
sys.path.append("/home/yoav/rdnd/models/pips_main")
...
def run_model(model, rgbs, N, sw):
    ...
    grid_y, grid_x = pip_utils.basic.meshgrid2d(B, N_, N_, stack=False, norm=False, device=DEVICE)
    ...
    if sw is not None and sw.save_this:
        ...
        # visualize the input
        o1 = sw.summ_rgbs('inputs/rgbs', pip_utils.improc.preprocess_color(rgbs[0:1]).unbind(1))
        # visualize the trajs overlaid on the rgbs
        o2 = sw.summ_traj2ds_on_rgbs('outputs/trajs_on_rgbs', trajs_e[0:1], pip_utils.improc.preprocess_color(rgbs[0:1]), cmap='spring', linewidth=linewidth)
        ...
        # alternate vis
        sw.summ_traj2ds_on_rgbs2('outputs/trajs_on_rgbs2', trajs_e[0:1], vis_e[0:1], pip_utils.improc.preprocess_color(rgbs[0:1]))
        
        # animation of inference iterations
        rgb_vis = []
        for trajs_e_ in preds_anim:
            trajs_e_ = trajs_e_ + pad
            rgb_vis.append(sw.summ_traj2ds_on_rgb('', trajs_e_[0:1], torch.mean(pip_utils.improc.preprocess_color(rgbs[0:1]), dim=1), cmap='spring', linewidth=linewidth, only_return=True))
...
def main():
    ...
    exp_name = 'yoav_first_time_testing'  # (exp_name is used for logging notes that correspond to different runs)
    ...
    filenames = glob('/raid/datasets/emil_drones_png/*.png')
    filenames = sorted(filenames)[:200]
    ...
    while global_step < max_iters:
        ...
        try:
            ...
            with torch.no_grad():
                # run with (model, images of shape [B, T, C, H, W], number of points to track, writer)
                trajs_e = run_model(model, rgbs, N, sw_t)
                # imshow_torch(trajs_e[0, 0, :, 0].reshape(16, 16))
                # imshow_torch(trajs_e[0, 1, :, 0].reshape(16, 16))
