...
def pre_start_scrip(base_directory_for_global):
    ...
    files = sorted(glob(f"{base_directory}/*.png") + glob(f"{base_directory}/*.jpg"))
    ...
    try:
        ...
        try:
            ...
            if "stack expects each tensor to be equal size" in str(e):
                raise RuntimeError("ImagesNotInTheSameSize")
...
def chain_demo_main_interface(base_directory_of_images, base_output,output_full_frame="frames",output_seg="segmentation", output_crop="crop"):
    ...
    try:
        trajs_e, rgbs = main_old(frames, batch_size)
    except Exception as e:
        raise e
    ...
    trajs_e_not_normalized *= torch.tensor([w, h]).unsqueeze(0)
    ...
    #ToDo add flag to knows if to do denoise or not
    ###For avarging displaying etc...
    # # insert ecc
    #
...
#     #ToDo there are some limitations on selecting roi
    ...
    try:
        chain_demo_main_interface(base_directory_of_images, base_output)
    except Exception as e:
