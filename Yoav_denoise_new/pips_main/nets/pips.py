import sys
sys.path.append("/home/dudy/Nehoray/Yoav_denoise_new/pips_main")
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from functools import partial
import pip_utils.basic
import pip_utils.samp
import pip_utils.misc
...
def balanced_ce_loss(pred, gt, valid=None):
    ...
    pos_loss = pip_utils.basic.reduce_masked_mean(loss, pos*valid)
    neg_loss = pip_utils.basic.reduce_masked_mean(loss, neg*valid)
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += i_weight * pip_utils.basic.reduce_masked_mean(i_loss, valids)
...
class BasicEncoder(nn.Module):
    def __init__(self, input_dim=3, output_dim=128, stride=8, norm_fn='batch', dropout=0.0):
        ...
        if self.norm_fn == 'group':
            self.norm1 = nn.GroupNorm(num_groups=8, num_channels=self.in_planes).to(DEVICE)
            self.norm2 = nn.GroupNorm(num_groups=8, num_channels=output_dim*2).to(DEVICE)
            
        elif self.norm_fn == 'batch':
            self.norm1 = nn.BatchNorm2d(self.in_planes).to(DEVICE)
            self.norm2 = nn.BatchNorm2d(output_dim*2).to(DEVICE)

        elif self.norm_fn == 'instance':
            self.norm1 = nn.InstanceNorm2d(self.in_planes).to(DEVICE)
            self.norm2 = nn.InstanceNorm2d(output_dim*2).to(DEVICE)

        elif self.norm_fn == 'none':
            self.norm1 = nn.Sequential().to(DEVICE)
            
        self.conv1 = nn.Conv2d(input_dim, self.in_planes, kernel_size=7, stride=2, padding=3, padding_mode='zeros').to(DEVICE)
        self.relu1 = nn.ReLU(inplace=True)

        self.shallow = False
        if self.shallow:
            self.layer1 = self._make_layer(64,  stride=1).to(DEVICE)
            self.layer2 = self._make_layer(96, stride=2).to(DEVICE)
            self.layer3 = self._make_layer(128, stride=2).to(DEVICE)
            self.conv2 = nn.Conv2d(128+96+64, output_dim, kernel_size=1).to(DEVICE)
        else:
            self.layer1 = self._make_layer(64,  stride=1).to(DEVICE)
            self.layer2 = self._make_layer(96, stride=2).to(DEVICE)
            self.layer3 = self._make_layer(128, stride=2).to(DEVICE)
            self.layer4 = self._make_layer(128, stride=2).to(DEVICE)

            self.conv2 = nn.Conv2d(128+128+96+64, output_dim*2, kernel_size=3, padding=1, padding_mode='zeros').to(DEVICE)
            self.relu2 = nn.ReLU(inplace=True).to(DEVICE)
            self.conv3 = nn.Conv2d(output_dim*2, output_dim, kernel_size=1).to(DEVICE)
        
        self.dropout = None
        if dropout > 0:
            self.dropout = nn.Dropout2d(p=dropout).to(DEVICE)
    ...
    def _make_layer(self, dim, stride=1):
        layer1 = ResidualBlock(self.in_planes, dim, self.norm_fn, stride=stride).to(DEVICE)
        layer2 = ResidualBlock(dim, dim, self.norm_fn, stride=1).to(DEVICE)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fhid, fcorr, flow):
        B, S, D = flow.shape
        assert(D==3)
        flow_sincos = pip_utils.misc.get_3d_embedding(flow, 64, cat_coords=True)
...
class Pips(nn.Module):
    def __init__(self, S=8, stride=8, device=15):
        ...
        self.fnet = BasicEncoder(output_dim=self.latent_dim, norm_fn='instance', dropout=0, stride=stride).to(DEVICE)
        
        self.delta_block = DeltaBlock(input_dim=self.latent_dim, hidden_dim=self.hidden_dim, corr_levels=self.corr_levels, corr_radius=self.corr_radius, S=self.S).to(DEVICE)
        ...
        self.vis_predictor = nn.Sequential(
            ...
            # nn.Linear(self.latent_dim, self.latent_dim),
            # nn.GELU(),
            nn.Linear(self.latent_dim, 1).to(DEVICE),
        )
    ...
    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        ...
        if feat_init is None:
            ...
            ffeat = pip_utils.samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B, N, C
        else:
            ffeat = feat_init
        ...
        if sw is not None and sw.save_this:
            ...
            for s in range(S):
                if trajs_g is not None:
                    ...
                    kp = pip_utils.improc.draw_circles_at_xy(torch.cat([e_, g_], dim=1), H8, W8, sigma=1).squeeze(2)
                    kp = pip_utils.improc.seq2color(kp, colormap='onediff')
                else:
                    kp = pip_utils.improc.draw_circles_at_xy(coords[0:1,s,0:1], H8, W8, sigma=1).squeeze(2)
                    kp = pip_utils.improc.seq2color(kp, colormap='spring')
                kp = pip_utils.improc.back2color(kp)
                kp_vis.append(kp)
        ...
        if sw is not None and sw.save_this:
            ...
            fcps_ = pip_utils.basic.normalize(fcps_)
            for s in range(S):
