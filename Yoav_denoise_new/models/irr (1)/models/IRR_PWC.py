...
class PWCNet(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        x1_raw = input_dict[:, 0] / 255
        x2_raw = input_dict[:, 1] / 255

        # x1_raw = input_dict['input1'] / 255
        # x2_raw = input_dict['input2'] / 255
        ...
        flow_f = torch.zeros(batch_size, 2, h_x1, w_x1).float().to(x1_raw.device)
        flow_b = torch.zeros(batch_size, 2, h_x1, w_x1).float().to(x1_raw.device)
        occ_f = torch.zeros(batch_size, 1, h_x1, w_x1).float().to(x1_raw.device)
        occ_b = torch.zeros(batch_size, 1, h_x1, w_x1).float().to(x1_raw.device)
        ...
        output_dict['flow'] = flow_f  # upsample2d_as(flow_f, x1_raw, mode="bilinear") * (1.0 / self._div_flow)
        output_dict['occ'] = occ_f  # upsample2d_as(occ_f, x1_raw, mode="bilinear")

        # return output_dict
        conf_x, conf_y = None, None
        # # normalize occ to 0 -> 1
        # occ_f -= occ_f.min()
        # occ_f /= occ_f.max()
        occ_f = self.sigmoid(occ_f)
        return [flow_f, occ_f, conf_x, conf_y]
