...
from .gru import BasicUpdateBlock, BasicUpdateBlockNFrames,  GMAUpdateBlock, BasicUpdateBlockOcclusion, GMAUpdateBlockOcclusion, GMAUpdateBlockConfidence, BasicUpdateBlockConfidence, BasicUpdateBlock3Frames, GMAUpdateBlock3Frames, GMAUpdateBlockNFrames
...
class MemoryDecoderLayer3Frames(nn.Module):
    def __init__(self, dim, cfg):
        super(MemoryDecoderLayer3Frames, self).__init__()
        self.cfg = cfg
        self.patch_size = cfg.patch_size # for converting coords into H2', W2' space
    ...
    def forward(self, query, key, value, memory, coords1, size, size_h3w3):
        ...
        """
        x_global, k, v = self.cross_attend(query, key, value, memory, coords1, self.patch_size, size_h3w3)
        # change net size to have B as 2 and not 1, and then there will be no difference between the 2 versions
        B, C, H1, W1 = size
        C = self.cfg.query_latent_dim
        x_global = x_global.view(B, H1, W1, C).permute(0, 3, 1, 2)
        return x_global, k, v
...
class MemoryDecoder3Frames(nn.Module):
    def __init__(self, cfg):
        super(MemoryDecoder3Frames, self).__init__()
        dim = self.dim = cfg.query_latent_dim
        self.cfg = cfg

        self.flow_token_encoder = nn.Sequential(
            nn.Conv2d(81 * 3, dim, 1, 1),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1, 1)
        )
        self.flow_token_encoder2 = nn.Sequential(
            nn.Conv2d(81 * 3 * 2, 2 * dim, 1, 1),
            nn.GELU(),
            nn.Conv2d(2 * dim, 2 * dim, 1, 1)
        )
        self.proj = nn.Conv2d(256, 256, 1)
        self.depth = cfg.decoder_depth
        self.decoder_layer = MemoryDecoderLayer3Frames(dim, cfg)

        if self.cfg.gma:
            self.update_block = GMAUpdateBlock3Frames(self.cfg, hidden_dim=128)
            self.att = Attention(args=self.cfg, dim=128, heads=3, max_pos_size=160, dim_head=128)
        else:
            self.update_block = BasicUpdateBlock3Frames(self.cfg, hidden_dim=128)
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        """
            ...
            context: [B, D, H1, W1]
        """
        B, D, H1, W1 = context.shape
        cost_maps = data['cost_maps']
        coords0, coords1 = initialize_flow(context.view(1, B*D, H1, W1))
        coords2 = torch.clone(coords1)

        if flow_init is not None:
            # print("[Using warm start]")
            coords1 = coords1 + flow_init
            coords2 = coords2 + flow_init
        ...
        for idx in range(self.depth):
            ...
            cost_forward1 = self.encode_flow_token(cost_maps, coords1)
            cost_forward2 = self.encode_flow_token(cost_maps, coords2)
            cost_forward = torch.cat([cost_forward1, cost_forward2], dim=0)
            # cost_backward = self.reverse_cost_extractor(cost_maps, coords0, coords1)

            query = self.flow_token_encoder(cost_forward)
            query = query.permute(0, 2, 3, 1).contiguous().view(1 * size[2] * size[3], 2, self.dim)
            cost_global, key, value = self.decoder_layer(query, key, value, cost_memory, coords1, size, data['H3W3'])
            if self.cfg.only_global:
                corr = cost_global
            else:
            ...
            flow2 = coords2 - coords1
            flow = torch.cat([flow1, flow2], dim=0)
            ...
            coords2 = coords2 + delta_flow[1:2]
            flow_up1 = self.upsample_flow(coords1 - coords0, up_mask[0:1])
            flow_up2 = self.upsample_flow(coords2 - coords1, up_mask[1:2])
            flow_up = torch.cat([flow_up1, flow_up2], dim=0)
            flow_predictions.append(flow_up)
...
class MemoryDecoderNFrames(nn.Module):
    def __init__(self, cfg, N):
        super(MemoryDecoderNFrames, self).__init__()
        dim = self.dim = cfg.query_latent_dim
        self.N = N
        self.cfg = cfg

        self.flow_token_encoder = nn.Sequential(
            nn.Conv2d(81 * (self.N-1), dim, 1, 1),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1, 1)
        )
        self.proj = nn.Conv2d(256, 256, 1)
        ...
        if self.cfg.gma:
            self.update_block = GMAUpdateBlockNFrames(self.cfg, hidden_dim=128, N=self.N-1)
            self.att = Attention(args=self.cfg, dim=128, heads=self.N-1, max_pos_size=160, dim_head=128)
        else:
            self.update_block = BasicUpdateBlockNFrames(self.cfg, hidden_dim=128, N=self.N-1)
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        """
            ...
            context: [B, D, H1, W1]
        """
        context = torch.cat([c for c in context], dim=0)
        B, D, H1, W1 = context.shape
        cost_maps = data['cost_maps']
        coords = []
        # coords0, coords1 = initialize_flow(context.view(1, B*D, H1, W1))
        coords0, coords1 = initialize_flow(context[0:1])
        coords = [coords0, coords1]
        for i in range(self.N - 2):
            coords.append(torch.clone(coords1))

        coords = torch.cat([c for c in coords], dim=0)
        # flow = coords1
        ...
        for idx in range(self.depth):
            ...
            costs_forward = []
            for c in coords[1:]:
                cost_forward = self.encode_flow_token(cost_maps, c.unsqueeze(0))
                costs_forward.append(cost_forward)

            costs_forward = torch.cat([c for c in costs_forward], dim=0)
            # cost_backward = self.reverse_cost_extractor(cost_maps, coords0, coords1)

            query = self.flow_token_encoder(costs_forward)
            # query = query.permute(0, 2, 3, 1).contiguous().view(size[0] * size[2] * size[3], 1, self.dim)
            query = query.permute(0, 2, 3, 1).contiguous().view(size[0] * size[2] * size[3], self.N-1, self.dim)
            # query = query.mean(1).unsqueeze(1)
            cost_global, key, value = self.decoder_layer(query, key, value, cost_memory, coords[-2:-1], size, data['H3W3'])
            if self.cfg.only_global:
                corr = cost_global
            else:
                corr = torch.cat([cost_global, costs_forward], dim=1)

            flows = []
            for [c1, c2] in zip(coords[1:], coords):
                f = c1 - c2
                flows.append(f)

            flows = torch.cat([f.unsqueeze(0) for f in flows], dim=0)

            if self.cfg.gma:
                net, up_mask, delta_flow = self.update_block(net, inp, corr, flows, attention)
            else:
                net, up_mask, delta_flow = self.update_block(net, inp, corr, flows)

            # flow = delta_flow
            for id, c in enumerate(coords[1:]):
                coords[id] += delta_flow[id-1]

            flows_up = []
            for id, [c1, c2] in enumerate(zip(coords[1:], coords)):
                flow_up = self.upsample_flow((c1 - c2).unsqueeze(0), up_mask[id:id+1])
                flows_up.append(flow_up)

            flows_up = torch.cat([f for f in flows_up], dim=0)
            flow_predictions.append(flows_up)
...
class MemoryDecoderOcclusion(nn.Module):
    def __init__(self, cfg):
        super(MemoryDecoderOcclusion, self).__init__()
        dim = self.dim = cfg.query_latent_dim
        self.cfg = cfg
        ...
        self.upscaling_occ = nn.Upsample(scale_factor=8)

        if self.cfg.gma:
            self.update_block = GMAUpdateBlockOcclusion(self.cfg, hidden_dim=128)
            self.att = Attention(args=self.cfg, dim=128, heads=1, max_pos_size=160, dim_head=128)
        else:
            self.update_block = BasicUpdateBlockOcclusion(self.cfg, hidden_dim=128)
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        for idx in range(self.depth):
            ...
            if self.cfg.gma:
                net, up_mask, delta_flow, occlusion_estimate = self.update_block(net, inp, corr, flow, attention)
            else:
                net, up_mask, delta_flow, occlusion_estimate = self.update_block(net, inp, corr, flow)

            # TODO: do we want to do some refinment on occlusion estimate????
            # flow = delta_flow
            coords1 = coords1 + delta_flow
            occlusion_estimate = self.upscaling_occ(occlusion_estimate)
            # delta flow is alwyas 1/8 the size of original flow, we can upsample somehow and reshape
            flow_up = self.upsample_flow(coords1 - coords0, up_mask)
            flow_predictions.append(flow_up)

        # take the last occlusion estimate
        # B, _, H, W = flow_up.shape
        # occlusion_estimate.reshape(B, 1, H, W)


        # if self.training:
        #     return flow_predictions , occlusion_estimate
        # else:
        #     return flow_predictions[-1], coords1-coords0, occlusion_estimate
        return flow_predictions[-1], occlusion_estimate




class MemoryDecoderConfidence(nn.Module):
    def __init__(self, cfg):
        super(MemoryDecoderConfidence, self).__init__()
        dim = self.dim = cfg.query_latent_dim
        self.cfg = cfg
        ...
        if self.cfg.gma:
            self.update_block = GMAUpdateBlockConfidence(self.cfg, hidden_dim=128)
            self.att = Attention(args=self.cfg, dim=128, heads=1, max_pos_size=160, dim_head=128)
        else:
            self.update_block = BasicUpdateBlockConfidence(self.cfg, hidden_dim=128)
    ...
    def forward(self, cost_memory, context, data={}, flow_init=None):
        ...
        for idx in range(self.depth):
            ...
            if self.cfg.gma:
                net, up_mask, delta_flow, confidence_estimate = self.update_block(net, inp, corr, flow, attention)
            else:
                net, up_mask, delta_flow, confidence_estimate = self.update_block(net, inp, corr, flow)

            # TODO: do we want to do some refinment on occlusion estimate????
            # flow = delta_flow
            coords1 = coords1 + delta_flow
            confidence_estimate = self.upscaling_occ(confidence_estimate) #TODO: should we use upscale flow for confidence???
            # delta flow is alwyas 1/8 the size of original flow, we can upsample somehow and reshape
            flow_up = self.upsample_flow(coords1 - coords0, up_mask)
            flow_predictions.append(flow_up)
        ...
        # else:
        #     return flow_predictions[-1], coords1-coords0, occlusion_estimate
        return flow_predictions[-1], confidence_estimate
