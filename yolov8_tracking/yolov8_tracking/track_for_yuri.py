...
def run(
        ...
        # "best_3.pt", #"best_1.pt",    #'yolov8s-seg.pt', #"best_1.pt", # model.pt path(s),  # /home/dudy/Nehoray/SHABACK_POC_NEW/object_tracking/git_repos/yolov8_tracking/weights/yolov8s-seg.pt
        ...
        conf_thres=0.0,  # confidence threshold
        ...
        for i, det in enumerate(prediction_outputs):  # detections per image
            ...
            if det is not None and len(det):
                if flag_perform_segmentation:
                    ...
                    if retina_masks:
                        ...
                        masks.append(process_mask_native(proto[i], det[:, 6:], det[:, :4],
                                                         current_frame_original.shape[:2]))  # HWC
                    else:
                        masks.append(
                            process_mask(proto[i], det[:, 6:], det[:, :4], current_frame_after_downsample.shape[2:],
                                         upsample=True))  # HWC
                        det[:, :4] = scale_boxes(current_frame_after_downsample.shape[2:], det[:, :4],
                                                 shape).round()  # rescale boxes to current_frame_original size
                else:
                ...
                if len(outputs[i]) > 0:
                    ...
                    if flag_perform_segmentation:
                        ...
                        annotator.masks(
                            ...
                            im_gpu=torch.as_tensor(current_frame_original, dtype=torch.float16).to(device).permute(2, 0,
                                                                                                                   1).flip(
                                0).contiguous() /
                            ...
                            if save_crop:
                                ...
                                save_one_box(np.array(bbox, dtype=np.int16), current_frame_original_copy,
                                             file=save_dir / 'crops' / txt_file_name / names[
                                                 c] / f'{id}' / f'{path.stem}.jpg', BGR=True)
            ...
            if show_vid:
                ...
                if frame_idx == 0:
                    ...
                    cv2.rectangle(image_result_counters, (x_plot_text, x_plot_text),
                                  (x_plot_text + w_plot_text * 10, y_plot_text + h_plot_text), (0, 0, 0), -1)
                    cv2.putText(current_frame_original, text=total_counting_text,
                                org=(x_plot_text + int(w_plot_text / 10), y_plot_text + int(h_plot_text / 1.5)),
...
def parse_opt():
    ...
    parser.add_argument('--project_path_to_save_results', default=ROOT / 'runs' / 'track',
...
def main():
    ...
    classes = [0, 1, 2, 3, 4, 5, 6, 7]  # EILON-COMBINED
