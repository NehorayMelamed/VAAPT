...
sys.path.append("/home/nehoray/PycharmProjects/Shaback/yolov8_tracking/yolov8_tracking_old/")

# This will prioritize modules/packages in '/full/path/to/directory'


from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.nn.autobackend import AutoBackend
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.data.dataloaders.stream_loaders import LoadImages, LoadStreams
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.data.utils import IMG_FORMATS, VID_FORMATS
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils import DEFAULT_CFG, LOGGER, SETTINGS, callbacks, colorstr, ops
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils.checks import check_file, check_imgsz, check_imshow, print_args, check_requirements
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils.files import increment_path
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils.torch_utils import select_device
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils.ops import Profile, non_max_suppression, scale_boxes, process_mask, \
    process_mask_native
from yolov8_tracking.yolov8_tracking.yolov8.ultralytics.yolo.utils.plotting import Annotator, colors, save_one_box
...
def run(
        ...
        # project_path_to_save_results = PARAMETER.CAR_LICENSE_PLATE_RECOGNITION,
        ...
        project_path_to_save_results = os.path.join(project_path_to_save_results,os.path.basename(source).split('.')[0])
        save_dir = increment_path(Path(project_path_to_save_results), exist_ok=exist_ok)  # increment run
        ...
        mask_path = Path(save_dir / 'masks')
        frames_path = Path(save_dir / 'frames')
        ...
        for frame_idx, batch in enumerate(dataset):
                                ...
                                if save_crop:
                                    ...
                                    save_one_box(np.array(bbox, dtype=np.int16), current_frame_original_copy, gain=1,
                                                 ...
                                                 file=save_dir / 'crops' / txt_file_name / names[
                                                     c] / f'{id}' / f'{frame_idx}.png', BGR=True, desired_size=(400,320))




                                    # add crop to the masks
            ...
            cv2.imwrite(f'{frames_path}/{frame_idx}.png', current_frame_original_copy)
        ...
        try:
            ...
            print(f"got expectation, - {e}, continue")
        LOGGER.info(
...
def interface_run(source,
    ...
    run(
        ...
        project_path_to_save_results=PARAMETER.DETECTION_DIRECTORY_PATH
,
        name='exp',  # save results to project/name
        exist_ok=False,  # existing project/name ok, do not increment
        line_thickness=2,  # bounding box thickness (pixels)
    ...
    # time.sleep(1)
    # base_crop_directory_path = os.path.join(PARAMETER.DETECTION_DIRECTORY_PATH, os.path.basename(source).split('.')[0])
                                    ...
                                    for sub_sub_sub_dir_name in os.listdir(sub_sub_dir_path):
                                        ...
                                        if os.path.isdir(sub_sub_sub_dir_path):
                                            ...
                                            for file in os.listdir(sub_sub_sub_dir_path):
                                                if file != "license_plate.jpg" and file.endswith(".png"):
                                                    list_of_jpg_files.append(os.path.join(sub_sub_sub_dir_path, file))
                                            # print(list_of_jpg_files)
                                            video_from_tensor(list_of_files=list_of_jpg_files, output_video=f"{sub_sub_sub_dir_path}/TYPE_{dir_name}_ID_{sub_sub_sub_dir_name}.mp4", size=[256,256], file_type=".png")
