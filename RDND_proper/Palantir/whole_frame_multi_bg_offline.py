...
from RapidBase.Utils.Tensor_Manipulation.Array_Tensor_Manipulation import crop_torch_batch, torch_to_numpy, crop_numpy_batch, \
    numpy_array_to_video_ready, scale_array_stretch_hist
from RapidBase.Utils.MISCELENEOUS import string_rjust
from RapidBase.Utils.IO.Path_and_Reading_utils import read_image_filenames_from_folder
from RapidBase.Utils.IO.Imshow_and_Plots import imshow_torch_video, imshow_torch
from Palantir.palantir_optional_stabilizations import align_palantir_CC_ECC_CC, align_palantir_CC_ECC_CC_pytorch
...
def initialize_experiment(original_frames_folder, results_path, experiment_name, sub_experiment_idx,
    ...
    experiment_dict.number_of_frames_per_bg = 1000
...
# ----- Phase III: Stabilize batch and extract outliers -----
def stabilize_batch(input_tensor, flag_estimate_new_bg, input_range, bg_range, experiment_dict,
                    ECC_warp_matrix=torch.eye(3, 3)):
    # --- I. Get BG Estimation ---
    ...
    aligned_tensor = []
    # research purpose
    num_iterations_list_ecc_global = []
    delta_shift_list_ecc_global = []
    ...
    for sub_batch_index in range(0, num_sub_batches):
        ...
        _, _, sub_aligned_tensor, H_matrix, num_iterations_list_ecc, delta_shift_list_ecc = \
            align_palantir_CC_ECC_CC_pytorch(input_tensor=sub_input_tensor,
                                             reference_tensor=bg_tensor,
                                             ECC_warp_mode='homography',
                                             ECC_warp_matrix=ECC_warp_matrix,
                                             ECC_iterations=50)
        # sum up results to total result
        del _
        last_ecc_h_matrix = H_matrix[-1]
        sub_aligned_tensor = pad_torch_batch(sub_aligned_tensor, (alignment_h, alignment_w))
        aligned_tensor += [sub_aligned_tensor]
        num_iterations_list_ecc_global += num_iterations_list_ecc
        delta_shift_list_ecc_global += delta_shift_list_ecc
    ...
    outliers_tensor_1 = find_outliers(aligned_tensor[: T // 2], bg_tensor.cuda(), noise_mult_factor=10).cpu()
    outliers_tensor_2 = find_outliers(aligned_tensor[T // 2:], bg_tensor.cuda(), noise_mult_factor=10).cpu()
    ...
    if flag_estimate_new_bg:
        ...
        estimate_bg(experiment_dict=experiment_dict,
                    ...
                    flag_calculate_shifts=True)  # homography doesn't enable proper shifts extraction
        last_ecc_h_matrix = torch.eye(3, 3)

    # --- VII. Return Results ---
    return last_ecc_h_matrix, num_iterations_list_ecc_global, delta_shift_list_ecc_global
...
if __name__ == '__main__':
    for experiment_idx in range(1, 20):
        ...
        bg_range = input_range
        # initialize h matrix to give warm start for homography calculation
        last_ecc_h_matrix = torch.eye(3, 3)
        # research purpose
        num_iterations_ecc_global = []
        delta_shift_ecc_global = []
        for batch_index in range(num_batches):
            ...
            flag_estimate_new_bg = (start_video != 0) and ((start_video % experiment_dict.number_of_frames_per_bg) < batch_size)
            last_ecc_h_matrix, num_iterations_ecc, delta_shift_ecc = stabilize_batch(video_tensor,
                                                                                 flag_estimate_new_bg=flag_estimate_new_bg,
                                                                                 input_range=input_range,
                                                                                 bg_range=bg_range,
                                                                                 experiment_dict=experiment_dict,
                                                                                 ECC_warp_matrix=last_ecc_h_matrix)
            if flag_estimate_new_bg:
                bg_range = input_range
            num_iterations_ecc_global += num_iterations_ecc
            delta_shift_ecc_global += delta_shift_ecc
            torch.cuda.empty_cache()

        plt.plot(np.linspace(1, len(num_iterations_ecc_global), len(num_iterations_ecc_global)), num_iterations_ecc_global)
        plt.plot(np.linspace(1, len(delta_shift_ecc_global), len(delta_shift_ecc_global)), delta_shift_ecc_global)
