import sys
sys.path.extend(['/home/omerl/rdnd','/home/omerl/FTVSR'])
...
from mmedit.models.backbones.sr_backbones.dct import dct_layer, reverse_dct_layer, check_and_padding_imgs, remove_image_padding, resize_flow
        ...
        for i in range(t - 1, -1, -1):
            ...
            if i < t - 1:  # no warping required for the last timestep
                ...
                location_update = flow_warp(location_update, flow.permute(0, 2, 3, 1), padding_mode='border',
                                            interpolation="nearest")  # n , 2t , h , w
                ...
                feat_prop = self.LTAM(lr_curr_feat, index_feat_buffer_s1, feat_prop, sparse_feat_buffer_s1,
                                      sparse_feat_buffer_s2, sparse_feat_buffer_s3, location_update)
            ...
            if i in keyframe_idx_backward:
                ...
                # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
                index_feat_prop_s1 = F.unfold(lr_curr_feat, kernel_size=(self.stride, self.stride), padding=0,
                                              stride=self.stride)
                # bs * (c*4*4) * (h//4*w//4) -->  bs * (c*4*4) * h//4 * w//4
                ...
                sparse_feat_prop_s2 = F.unfold(feat_prop, kernel_size=(int(1.5 * self.stride), int(1.5 * self.stride)),
                ...
                sparse_feat_prop_s2 = F.fold(sparse_feat_prop_s2, output_size=(int(1.5 * h), int(1.5 * w)),
                                             kernel_size=(int(1.5 * self.stride), int(1.5 * self.stride)), padding=0,
                                             stride=int(1.5 * self.stride))
                # bs * c * (h*2) * (w*2) -->  bs * c * h * w
                sparse_feat_prop_s2 = F.adaptive_avg_pool2d(sparse_feat_prop_s2, (h, w))
                # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
                sparse_feat_prop_s2 = F.unfold(sparse_feat_prop_s2, kernel_size=(self.stride, self.stride), padding=0,
                                               stride=self.stride)
                # bs * (c*4*4) * (h//4*w//4) -->  bs * (c*4*4) * h//4 * w//4
                ...
                # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
                sparse_feat_prop_s3 = F.unfold(sparse_feat_prop_s3, kernel_size=(self.stride, self.stride), padding=0,
                                               stride=self.stride)
                # bs * (c*4*4) * (h//4*w//4) -->  bs * (c*4*4) * h//4 * w//4
...
class FTT(nn.Module):
    ...
    def forward(self, bicubic_imgs, high_frequency_imgs, flows, padiings, to_cpu=False):
        ...
        # resize flows
        if flows_forward is not None:
            flows_forward = resize_flow(flows_forward.view(-1, 2, h, w), size_type='shape',
                                        sizes=(h // self.dct_kernel[0], w // self.dct_kernel[1]))
            flows_forward = flows_forward.view(n, t - 1, 2, h // self.dct_kernel[0], w // self.dct_kernel[1])
        flows_backward = resize_flow(flows_backward.view(-1, 2, h, w), size_type='shape',
                                     sizes=(h // self.dct_kernel[0], w // self.dct_kernel[1]))
        ...
        dct_bic_fea = self.feat_extractor(self.conv_layer1(dct_bic)).view(n, t, 512, h // self.dct_kernel[0],
                                                                          w // self.dct_kernel[1])
        dct_hfi_fea = self.feat_extractor(self.conv_layer1(dct_hfi)).view(n, t, 512, h // self.dct_kernel[0],
                                                                          w // self.dct_kernel[1])
...
class FTTA_layer(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        qs = torch.nn.functional.unfold(qs, self.patch_k, dilation=1, padding=0,
                                        stride=self.patch_stride)  # (N*64, 3*8*8, num)
        ks = torch.nn.functional.unfold(ks, self.patch_k, dilation=1, padding=0,
                                        stride=self.patch_stride)  # (N*64, 3*8*8, num)
        vs = torch.nn.functional.unfold(vs, self.patch_k, dilation=1, padding=0,
                                        stride=self.patch_stride)  # (N*64, 3*8*8, num)
        ...
        out = torch.nn.functional.fold(out, (H, W), self.patch_k, dilation=1, padding=0,
                                       stride=self.patch_stride)  # (batch*64, 3, H, W)
...
# a = FTVSR()
#
# b = torch.load(cp2)['state_dict']
# len(b)
#
# a.load_state_dict(new_dict)
# # amaizing just document it
# step_counter = b['step_counter']
#
# new_dict = {}
#
# for i, bb in enumerate(b):
#     if i > 0:  # don't include step_counter
#         new_dict[bb.split('generator')[1][1:]] = b[bb]
#         if i % 50 == 0:
#             print(i)