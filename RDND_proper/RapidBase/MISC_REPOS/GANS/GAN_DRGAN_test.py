...
def compute_gradient_penalty(Discriminator, X_input_image_batch):
    """Calculates the gradient penalty loss for DRAGAN"""
    # Random weight term for interpolation
    alpha = Tensor(np.random.random(size=X_input_image_batch.shape))

    # Take the input image batch and add noise to it:
    interpolates =  alpha * X_input_image_batch + ((1 - alpha) * (X_input_image_batch + 0.5 * X_input_image_batch.std() * torch.rand(X_input_image_batch.size())))
    interpolates = Variable(interpolates, requires_grad=True)

    # Pass the noisy images into the Discriminator:
    d_interpolates = Discriminator(interpolates)

    # Fake label (1):
    fake = Variable(Tensor(X_input_image_batch.shape[0], 1).fill_(1.0), requires_grad=False)
