...
import cv2
from RapidBase.Utils.Tensor_Manipulation.Array_Tensor_Manipulation import torch_get_4D, torch_get_2D, torch_get_3D, torch_get_5D, any_data_type_to_torch
# from RapidBase.Utils.Tensor_Manipulation.Pytorch_Numpy_Utils import get_full_shape_torch, get_shape
from RapidBase.Utils.Tensor_Manipulation.Array_Tensor_Manipulation import get_full_shape_torch


def bilinear_interpolate_torch(input_tensor, X, Y):
...
class Warp_Tensors_Affine_Layer(nn.Module):
    def __init__(self, *args):
        ...
        self.X0 = None
        self.Y0 = None
    def forward(self, input_tensors, shift_x, shift_y, scale, rotation_angle, return_flow_grid=False, flag_return_only_flow_grid=False, flag_interpolation_mode='bilinear'):
        ...
        if flag_new_meshgrid:
            self.T, self.C, self.H, self.W = input_tensors.shape  #TODO: right now assuming 4D input, change this!!!!!
            (self.B, self.T, self.C, self.H, self.W), shape_length, shape_vec = get_full_shape_torch(input_tensors)
            # (1). Create meshgrid:
            ...
            self.X0 = self.X0.unsqueeze(0).unsqueeze(-1)
            self.Y0 = self.Y0.unsqueeze(0).unsqueeze(-1)
            # # (3). Duplicate meshgrid for each batch Example. TODO: do i really need to do this?
            # self.X0 = torch.cat([self.X0] * self.B, 0)
            # self.Y0 = torch.cat([self.Y0] * self.B, 0)
            # Duplicate according to the number of instances (TODO: transfer this to BxT):
            max_length = 0
            max_length = max(max_length, len(shift_x))
            max_length = max(max_length, len(shift_y))
            max_length = max(max_length, len(rotation_angle))
            max_length = max(max_length, len(scale))
            self.X0 = torch.cat([self.X0] * max_length, 0)
            self.Y0 = torch.cat([self.Y0] * max_length, 0)

        #TODO: add default which avoid calcualtions: for instance is scale=None then don't calculate it and then assign scale=1 for good measure
        shift_x_tensor = torch_get_4D(any_data_type_to_torch(shift_x, input_tensors.device), 'T').to(input_tensors.device)
        shift_y_tensor = torch_get_4D(any_data_type_to_torch(shift_y, input_tensors.device), 'T').to(input_tensors.device)
        rotation_angle_tensor = torch_get_4D(any_data_type_to_torch(rotation_angle, input_tensors.device), 'T').to(input_tensors.device)
        scale_tensor = torch_get_4D(any_data_type_to_torch(scale, input_tensors.device), 'T').to(input_tensors.device)
        ...
        ### Warp: ###
        if flag_return_only_flow_grid == False:
        ...
        #TODO: allow only returning the flow_grid, without calculating the output_tensor
        if flag_return_only_flow_grid:
            return None, flow_grid
        elif return_flow_grid:
