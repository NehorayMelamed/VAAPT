...
def update_transform_params_torch(H_matrix_in, delta_p, transform_string):
    ...
    if transform_string == 'homography':
        ...
        delta_p_reshaped = torch_reshape_image(delta_p, (3, 3), order='F')
        H_matrix_out = H_matrix_in + delta_p_reshaped
        H_matrix_out[2, 2] = 1
...
class ECC_Layer_Torch(nn.Module):
    ...
    def get_jacobian_for_warp_transform_torch(self, x_vec, y_vec, H_matrix, Jx, Jy, J0, J1, transform_string, H, W):
        ...
        if str.lower(transform_string) == 'homography':
            ### New, Better Way: ###
            # ### V1: ###
            # #TODO: i can do even better still!!! Jx/den is calculated below and below there is H_matrix[0, 0]*Jx/den + H_matrix[0, 1]*Jy/den + H_matrix[0,2]/den !!!!
            # den = (H_matrix[-1, 0] * Jx + H_matrix[-1, 1] * Jy + H_matrix[-1, 2])
            # xy_prime_reshaped_X = (H_matrix[0, 0] * Jx + H_matrix[0, 1] * Jy + H_matrix[0, 2]) / den
            # xy_prime_reshaped_Y = (H_matrix[1, 0] * Jx + H_matrix[1, 1] * Jy + H_matrix[1, 2]) / den
            # Jx = Jx / den  # element-wise
            # Jy = Jy / den  # element-wise
            ### V2: ###
            den = (H_matrix[-1, 0] * Jx + H_matrix[-1, 1] * Jy + H_matrix[-1, 2])
            ...
            xy_prime_reshaped_X = H_matrix[0, 0] * Jx + H_matrix[0, 1] * Jy + (H_matrix[0, 2]*denom_inverse)
            xy_prime_reshaped_Y = H_matrix[1, 0] * Jx + H_matrix[1, 1] * Jy + (H_matrix[1, 2]*denom_inverse)
...
def ECC_demo_numpy():
    ...
    reference_tensor = read_image_torch(path1)
    input_tensor = read_image_torch(path2)
    input_tensor = RGB2BW(input_tensor[0]).cuda()[0]
    reference_tensor = RGB2BW(reference_tensor[0]).cuda()[0]
    ECC_layer_object = ECC_Layer_Torch(reference_tensor, number_of_iterations_per_level=5, number_of_levels=1, transform_string='homography', delta_p_init=delta_p_init)
    tic()
    H_matrix, warpedImage = ECC_layer_object.forward(reference_tensor, input_tensor, delta_p_threshold=0)
    toc()
