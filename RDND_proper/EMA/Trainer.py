...
from PARAMETER import path_ema_ckpt
...
class Model:
    ...
    def load_model(self, name=None, rank=0):
        ...
        if rank <= 0 :
            if name is None:
                name = self.name
            self.net.load_state_dict(convert(torch.load(f'{path_ema_ckpt}/{name}.pkl')))
    
    def save_model(self, rank=0):
        if rank == 0:
            torch.save(self.net.state_dict(),f'{path_ema_ckpt}/{self.name}.pkl')
