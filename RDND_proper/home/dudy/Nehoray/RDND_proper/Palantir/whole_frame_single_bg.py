...
from RapidBase.Utils.Tensor_Manipulation.Array_Tensor_Manipulation import crop_torch_batch, torch_to_numpy, \
    crop_numpy_batch, numpy_array_to_video_ready, scale_array_stretch_hist
...
def stabilize_batch(input_tensor, experiment_dict, last_ecc_h_matrix=torch.eye(3, 3)):
    # --- I. Get BG Estimation ---
    ...
    for sub_batch_index in range(0, num_sub_batches):
        ...
        _, _, sub_aligned_tensor, ECC_H_matrix, shift_h, shift_w, rotation = \
            align_palantir_CC_ECC_CC(input_tensor=sub_input_tensor,
                                     reference_tensor=bg_tensor,
                                     ECC_warp_mode='homography',
                                     ECC_warp_matrix=last_ecc_h_matrix,
                                     ECC_iterations=40)
        del _
        # set warm start for the next iteration
        last_ecc_h_matrix = ECC_H_matrix[-1]
    ...
    # --- VII. Return Results ---
    return input_tensor, bg_tensor, aligned_tensor, outliers_tensor, diff_tensor, last_ecc_h_matrix


if __name__ == '__main__':
    for experiment_idx in range(2, 20):
        ...
        for batch_index in range(num_batches):
            ...
            _, _, _, _, _, last_ecc_h_matrix = stabilize_batch(video_tensor, experiment_dict, last_ecc_h_matrix)
            del _
            torch.cuda.empty_cache()