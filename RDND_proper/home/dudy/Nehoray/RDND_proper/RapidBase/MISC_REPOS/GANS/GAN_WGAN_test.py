...
for current_epoch in range(OPT.number_of_epochs):
    for current_batch_index, (current_image_batch, _) in enumerate(dataloader):
        ...
        ###########################################################################################################################################################
        ###  Train Discriminator  - First Train the Discriminator and only then the Generator and only after OPT.discriminator_to_generator_training_loop_ratio ###
        optimizer_D.zero_grad()
        # Sample noise as generator input
        z = Variable(Tensor(np.random.normal(0, 1, (batch_size, OPT.latent_space_dimensionality))))
        # Generate a batch of images
        fake_imgs = generator(z).detach()
        # Adversarial loss - very simple loss!!!! not even LS
        ...
        ########################
        ###  Train Generator ###
        if number_of_batches_so_far % OPT.discriminator_to_generator_training_loop_ratio == 0:
        ...
        print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]" % (current_epoch, OPT.number_of_epochs,
                                                                         number_of_batches_so_far % len(dataloader),
                                                                         len(dataloader),
