...
imgaug_parameters['affine_translation_number_of_pixels'] = {"x": (-5,5), "y": (-5,5)}
...
# train_dataloader.__iter__().__next__()
...
super_folder_to_save_result_videos = 'C:/Users\dkarl\PycharmProjects\dudykarl\TNR\Official Testing/Shift Images On The Fly/'
...
save_figures_folder = super_folder_to_save_result_videos + created_video_filename_prefix
    ...
    for batch_index, data_from_dataloader in enumerate(train_dataloader):
        ...
        for current_time_step in arange(0, number_of_time_steps):
            ...
            if current_time_step == 0:
                original_clean_numpy_tensor_form = current_clean_tensor.cpu().numpy()
                original_clean_numpy = np.transpose(current_clean_tensor.cpu().numpy(), [0,2,3,1])
                B, H, W, C = original_clean_numpy.shape
                start_H = int32((H - final_crop_size) / 2)
                stop_H = start_H + final_crop_size
                start_W = int32((W - final_crop_size) / 2)
                stop_W = start_W + final_crop_size
                original_clean_numpy_cropped = original_clean_numpy_tensor_form[:,:,start_H:stop_H,start_W:stop_W]
                original_tensor = torch.FloatTensor(original_clean_numpy_cropped);
                # original_tensor = original_tensor/255;
                original_tensor = original_tensor.to(Generator_device)


            ### Shift Tensors: ###
            current_image = original_clean_numpy.copy()
            deterministic_Augmenter_Object = imgaug_transforms.to_deterministic()
            current_image = deterministic_Augmenter_Object.augment_images(current_image);
            current_image = current_image[:,start_H:stop_H, start_W:stop_W, :]
            current_image = np.transpose(current_image, [0,3,1,2])
            current_clean_tensor = torch.Tensor(current_image)
            current_noisy_tensor = torch.Tensor(current_image)
            ...
            if current_time_step == 0 and number_of_time_steps_from_batch_start == 1:  # very start of batch
                ...
                averaged_image = noise_RGB_LinearShotNoise_Torch(original_tensor, gain=noise_gain, flag_output_uint8_or_float='float');
            else:
                ...
                current_noisy_tensor_for_averaging = noise_RGB_LinearShotNoise_Torch(original_tensor, gain=noise_gain, flag_output_uint8_or_float='float');
            ...
            #(1). L1:
            average_to_clean_L1 = L1_Loss_Function(averaged_image, original_tensor).item();  # between average image and Clean
            ...
            #(2). PSNR:
            average_to_clean_PSNR = PSNR_Loss_Function(averaged_image, original_tensor).item();  # between average image and Clean
            ...
            #(3). SSIM:
            average_to_clean_SSIM = SSIM_Loss_Function(averaged_image, original_tensor).item();  # between average image and Clean
