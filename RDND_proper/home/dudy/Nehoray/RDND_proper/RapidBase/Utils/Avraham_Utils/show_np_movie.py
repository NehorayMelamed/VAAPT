...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-v", "--video_directory_prefix", help="NP prefix", required=True)
...
def check_input(arguments: argparse.Namespace) -> None:
    if not os.path.exists(f"{arguments.video_file}_0.npy"):
...
if __name__ == '__main__' :
    ...
    current_frame = 0
    next_frame = f"{args.video_file}_{current_frame}.npy"
    while os.path.exists(next_frame):
        frame_viewer.show_frame(np.load(next_frame))
