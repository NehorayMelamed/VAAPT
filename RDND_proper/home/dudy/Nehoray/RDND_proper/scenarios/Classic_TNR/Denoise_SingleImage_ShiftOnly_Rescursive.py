...
### Results Paths: ###
results_path = '/home/mafat/PycharmProjects/IMOD/scenarios/TNR_Results/SingleImage_RigitShift'
...
max_search_area = 5 #for algorithms like NCC
SNR = 5
crop_size_x = 1024
crop_size_y = 1024
...
for frame_index in np.arange(1, number_of_images):
    ...
    shifts_array, clean_frame_previous_warped = register_images(noisy_frame_current_cropped, clean_frame_previous,  #I activtely shift noisy_frame_current_cropped, so that's why it's the second input
    ...
    shifts_x = shifts_array[0] * -1  #This is due to signs convention!!!!!
    shifts_y = shifts_array[1] * -1
    shifts_array = (shifts_x, shifts_y)
    ...
    image_to_write = torch.cat((clean_frame_current, clean_frame_simple_averaging/frame_index), -1)
    image_to_write = torch.cat((image_to_write, image_to_write, image_to_write), 1)
    image_to_write = image_to_write[0].cpu().numpy().transpose(1, 2, 0)
    image_to_write = (image_to_write * 255).clip(0, 255).astype(np.uint8)
    ...
    simple_average_frame_current_cropped_numpy = (clean_frame_simple_averaging[0].clamp(0,1).cpu().numpy().transpose(1, 2, 0)*255/frame_index).astype(uint8)
