...
### Results Paths: ###
results_path = '/home/mafat/PycharmProjects/IMOD/scenarios/TNR_Results/SingleImage_RigitShift_NonRecursive_Rolling'
...
rolling_original_frames_list = []
rolling_noisy_frames_list = []
for i in np.arange(output_frames_noisy_current.shape[0]):
    rolling_original_frames_list.append(original_frames_current[i:i+1,:,:,:])
    rolling_noisy_frames_list.append(output_frames_noisy_current[i:i+1,:,:,:])
...
for frame_index in np.arange(1, number_of_images):
    ...
    ### Get Last Frame: ###
    original_frame_previous = rolling_original_frames_list[-1]
    ...
    # original_frame_current = shift_layer_torch.forward(original_frame_previous, shiftx, shifty)  #TODO: shift previous frame (already shifted) or seed frame?
    original_frame_current = shift_layer_torch.forward(center_frame_original_current.data, shiftx, shifty)  #TODO: shift previous frame (already shifted) or seed frame?

    ### Add Noise: ###
    noisy_frame_current = original_frame_current + 1/sqrt(SNR)*torch.randn_like(original_frame_current)

    ### Take new image and add it as the latest frame in the images list: ###
    del rolling_original_frames_list[0]
    del rolling_noisy_frames_list[0]
    rolling_original_frames_list.append(original_frame_current)
    rolling_noisy_frames_list.append(noisy_frame_current)

    ### Make New torch tensor batch of images: ###
    output_frames_noisy_current = torch.cat(rolling_noisy_frames_list)
    original_frames_current = torch.cat(rolling_original_frames_list)
    center_frame_simple_averaging = 0
    for i in np.arange(len(rolling_noisy_frames_list)):
        center_frame_simple_averaging += rolling_noisy_frames_list[i]/len(rolling_noisy_frames_list)
    center_frame_noisy_current = rolling_noisy_frames_list[len(rolling_noisy_frames_list)//2]
    center_frame_original_current = rolling_original_frames_list[len(rolling_original_frames_list)//2]
    clean_frame_perfect_averaging = center_frame_original_current + 1 / sqrt(SNR) / sqrt(number_of_images_to_generate_each_time) * torch.randn_like(original_frame_current)
    ...
    ### Register Images (return numpy arrays) and get shifts: ###
