...
original_frames_folder = os.path.join(datasets_main_folder, '/KAYA_EXPERIMENT/3')
...
results_path_name_addition = 'KAYA4'
...
for frame_index in np.arange(1, number_of_images):
    ...
    original_frames_current = original_frames_output_dict.center_frame_original[0].data
    original_frames_current = original_frames_current.unsqueeze(0).unsqueeze(0) / normalization_factor_to_get_255_max
    ...
    ### Add Noise: ###
    noisy_frames_current = original_frames_current + 1/sqrt(SNR)*torch.randn_like(original_frames_current)

    ### Center Crop: ###
    noisy_frames_current_cropped = crop_torch_batch(noisy_frames_current, crop_size_x, crop_style='center')
    original_frames_current_cropped = crop_torch_batch(original_frames_current, crop_size_x, crop_style='center')
    original_frame_current_cropped = original_frames_current_cropped[:,:,:,-1:]
    noisy_frame_current_cropped = noisy_frames_current_cropped[:,:,:,-1:]
    ...
    ### For Movies That I Know Don't Shift: ###
    shifts_array = (0,0)
    clean_frame_previous_warped = clean_frame_previous  #(*). ASSUMING NO SHIFT!@#@!$@#%#@$@
    ...
    title('Inferred Sigma Mean: ' + str(int(float(decimal_notation((noise_map_current*255*256).mean(), 0)))) + ', ' +
          'Inferred SNR Mean: ' + decimal_notation(np.abs((clean_frame_current/(noise_map_current+1e-5))*1).mean(), 1) )
