...
experiment_name_addone = 'ShiftOnly_2'
# max_shifts_vec = np.flip(np.arange(10))
max_shifts_vec = my_linspace(0,2,20)
algorithm_names = ['MAOR', 'CrossCorrelation', 'NormalizedCrossCorrelation']  #TODO: Add optical flow step with confidence metric?
SNR_vec = [np.inf, 100, 10, 5, 4, 3, 2, 1]
crop_size_vec = [1024,512,256,128]
binning_vec = [4,2,1]
rotation_angle_vec = [0]  #[degrees]
scale_factor_vec = [1]
number_of_blur_iterations_per_pixel = 10
reference_to_current_image_SNR_ratio = np.sqrt(1)
reference_image_blur_ratio = 1
number_of_algorithms = 3
number_of_affine_parameters = 4
...
results_vec = np.zeros((len(clean_image_filenames), len(max_shifts_vec), len(rotation_angle_vec),
                        len(scale_factor_vec), len(crop_size_vec), len(binning_vec), len(SNR_vec),
                        number_of_algorithms, number_of_blur_options, number_of_affine_parameters)) #last two entries: with/without blur; number of affine params
shifts_vec = np.zeros((len(clean_image_filenames), len(max_shifts_vec), len(rotation_angle_vec),
                       len(scale_factor_vec), len(crop_size_vec), len(binning_vec), len(SNR_vec),
                       number_of_algorithms, number_of_blur_options, number_of_affine_parameters))
        ...
        for rotation_counter, current_rotation_angle in enumerate(rotation_angle_vec):
            for scale_counter, current_scale_factor in enumerate(scale_factor_vec):
                ...
                for blur_counter in np.arange(number_of_blur_options):
                    ...
                    max_pixels_shift = max(max_pixels_shift, (current_rotation_angle * pi / 180) * (sqrt(2) * np.max(crop_size_vec)))
                    ### 0==no blur, 1==with blur: ###
                    if blur_counter == 0:
                        current_image_reference = current_image.data
                        current_image_shifted = shift_layer_torch.forward(current_image.data, shiftx, shifty)
                        # current_image_shifted = shift_matrix_subpixel_torch(current_image.data, shiftx, shifty)
                    else:
                        ...
                        current_image_reference = blur_image_motion_blur_torch(current_image.data,
                                                                                    shiftx*reference_image_blur_ratio,
                                                                                    shifty*reference_image_blur_ratio,
                                                                                    number_of_blur_iterations, shift_layer_torch)
                        ### Blur Second Image: ###
                        #(1). shift to reference image blur location:
                        current_image_initial_shifted = shift_layer_torch.forward(current_image.data,
                                                                            shiftx*reference_image_blur_ratio,
                                                                            shifty*reference_image_blur_ratio,)
                        #(2). continue to blur image:
                        current_image_blurred = blur_image_motion_blur_torch(current_image_initial_shifted.data,
                                                                                    shiftx,
                                                                                    shifty,
                        ...
                        for SNR_counter, current_SNR in enumerate(SNR_vec):
                            ...
                            for binning_counter, current_binning in enumerate(binning_vec):
                                ...
                                for algorithm_counter in np.arange(0, number_of_algorithms):
                                    ...
                                    # try:  #(*). the try statement original was because some algorithms had runtime errors, and if i find a 0.0000 later on i know what happened

                                    ### Register Images (return numpy arrays): ###
                                    algorithm_name = algorithm_names[algorithm_counter]
                                    affine_parameters, clean_frame_previous_warped = \
                                        register_images(current_image_noisy_final.data, current_image_shifted_noisy_final.data,
                                                    algorithm_name,
                                                    search_area=7,
                                    ...
                                    if type(affine_parameters) == tuple:
                                        affine_parameters = list(affine_parameters)
                                    affine_parameters[0] = np.abs(affine_parameters[0]) * np.sign(shiftx)
                                    affine_parameters[1] = np.abs(affine_parameters[1]) * np.sign(shifty)
                                    ...
                                    for current_parameter in affine_parameters:
                                        if type(current_parameter) is np.ndarray:
                                            affine_parameters_list.append(current_parameter[0])
                                        else:
                                            affine_parameters_list.append(current_parameter)
                                    if len(affine_parameters_list) == 2:
                                        affine_parameters_list.append(1.0) # scale = 1
                                        affine_parameters_list.append(0.0) # rotation = 0
                                    affine_parameters_after_binning = []
                                    for current_parameter in affine_parameters_list:
                                        affine_parameters_after_binning.append(current_parameter * current_binning)
                                    affine_parameters_list = affine_parameters_after_binning
                                    ...
                                    ### Assign Results: ###
                                    # results_vec = np.zeros(
                                    #     (len(clean_image_filenames), len(max_shifts_vec), len(rotation_angle_vec),
                                    #      len(scale_factor_vec), len(crop_size_vec), len(binning_vec), len(SNR_vec),
                                    #      number_of_algorithms, number_of_blur_options, number_of_affine_parameters))
                                    ...
                                    results_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,0] = affine_parameters_list[0]
                                    results_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,1] = affine_parameters_list[1]
                                    results_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,2] = affine_parameters_list[2]
                                    results_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,3] = affine_parameters_list[3]
                                    shifts_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,0] = shiftx
                                    shifts_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,1] = shifty
                                    shifts_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,2] = current_scale_factor
                                    shifts_vec[image_counter, shift_counter, rotation_counter, scale_counter,
                                                crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter,3] = current_rotation_angle
                                    # except:
                                    #     print('problem')
    toc()
