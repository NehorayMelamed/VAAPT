...
folder_path = os.path.join(folder_path, 'Registration_Results')
filename_parameters_dict = os.path.join(folder_path, 'ShiftOnly_2_parameter_dict_known_shifts.pkl')
filename_results = os.path.join(folder_path, 'ShiftOnly_2_results_vec_known_shifts.npy')
filename_original_shifts = os.path.join(folder_path, 'ShiftOnly_2_shifts_vec_known_shifts.npy')
                    ...
                    for crop_counter, current_crop_size in enumerate(crop_size_vec):
                        for blur_counter in np.arange(2):
                            for algorithm_counter in np.arange(0, number_of_algorithms):
                                ...
                                for i in np.arange(clean_results_vec.shape[-1]):
                                    current_results_mat = clean_results_vec[:, shift_counter, rotation_counter, scale_counter,
                                                       crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter, i]
                                    current_shifts_mat = shifts_vec[:, shift_counter, rotation_counter, scale_counter,
                                                       crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter, i]
                                    current_error_median = np.median(np.abs(current_results_mat - current_shifts_mat))  #get median for current algorithm, current SNR, and current shift
                                    indices_above_threshold = (np.abs(current_results_mat - current_shifts_mat) > current_error_median * 3)
                                    current_results_mat[indices_above_threshold] = np.nan
                                    clean_results_vec[:, shift_counter, rotation_counter, scale_counter,
                                                       crop_counter, binning_counter, SNR_counter, algorithm_counter, blur_counter, i] = current_results_mat
                ...
                for crop_counter, current_crop_size in enumerate(crop_size_vec):
                    for blur_counter in np.arange(2):
                        for algorithm_counter in np.arange(0, number_of_algorithms + 1):
                            if algorithm_counter < number_of_algorithms:
