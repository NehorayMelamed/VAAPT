...
model_deblur_path = '/raid/Pytorch_Checkpoints/2022-07-11/Model_New_Checkpoints/VRT_deblur_tranin_drones_no_noise.py/VRT_deblur_tranin_drones_no_noise.py_TEST1_Step23400.tar'
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + 'deblur_pretrained'
...
IO_dict.number_of_image_frames_to_generate = 1
IO_dict.number_of_images_per_video_to_load = 1
### Miscellenous: ###
IO_dict.flag_normalize_images_to_PPP = True
IO_dict.PPP = [2, 10]  # Photons per Pixel.
### Noise Addition: ###
temp_factor = 20/255
...
IO_dict.transforms_dict.number_of_blur_steps_per_pixel = 3
### Super Resolution: ###
...
w,h,offset = 192,192,IO_dict.transforms_dict.shift_size*2 # 12,512,0
w,h = w-offset, h-offset
...
IO_dict.is_moving_images = True
IO_dict.num_moving_images = 6*3
### Test/Validation Dict: ###
w,h,offset = 1080,1920,IO_dict.transforms_dict.shift_size*2 # 12,512,0
w,h = w-offset, h-offset
...
IO_dict_test.number_of_image_frames_to_generate = 1
IO_dict_test.number_of_images_per_video_to_load = 1
IO_dict_test.num_moving_images = 6*3

#######################################################################################################################################
## DataSet & DataLoader Objects: ###
####################################
### DataSets: ###
train_dataset = TrainingCore.datasets.TrainingCore.datasets.DataSet_Videos_In_Folders_LoadCorruptedGT(Train_Images_Folder, IO_dict)
test_dataset = TrainingCore.datasets.TrainingCore.datasets.DataSet_Videos_In_Folders_LoadCorruptedGT(Test_Images_Folder, IO_dict_test)
...
# c = bla.output_frames_before_adding_noise
# imshow_torch_seamless(a[1])
# imshow_torch_seamless(b[0]/255)
# imshow_torch_video_seamless(b, number_of_frames=b.shape[0])
# imshow_torch_video_seamless(a, number_of_frames=a.shape[0])
# imshow_torch_video_seamless(c, number_of_frames=a.shape[0])
# plt.show(block=True)
