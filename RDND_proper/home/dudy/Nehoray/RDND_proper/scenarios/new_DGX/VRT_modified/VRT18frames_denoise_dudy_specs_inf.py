...
model_denoise_path = '/raid/Pytorch_Checkpoints/2022-06-26/Model_New_Checkpoints/VRT_18_frames_denoise_dudy_specs_for_rosh_mop.py(harder_reg)' \
                     '/VRT_18_frames_denoise_dudy_specs_for_rosh_mop.py(harder_reg)_TEST1_Step7800.tar'
...
#(2). Train Images:
Train_Images_Folder = osp.join(datasets_main_folder, 'drones/T/all')
...
Test_Images_Folder = osp.join(datasets_main_folder, 'drones/T/all')
...
Train_dict.number_of_validation_samples = 12
...
IO_dict.test_args = TestArgs(tile=[18,192,192], nonblind_denoising=True)
...
IO_dict.PPP = 3 #[2, 10]  # Photons per Pixel.
### Noise Addition: ###
temp_factor = 70/255
...
IO_dict.noise_dict.per_pixel_readout_noise_sigma = (5*temp_factor)
...
IO_dict.noise_dict.flag_add_per_pixel_NU = True # 5+6)
IO_dict.noise_dict.flag_add_row_NU = True # 7+8)
IO_dict.noise_dict.flag_add_col_NU = True # 9+10)
...
IO_dict_test.number_of_image_frames_to_generate = 240
IO_dict_test.number_of_images_per_video_to_load = 240
#######################################################################################################################################
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames_checkpoint(False, checkpoint_path=model_denoise_path, train_device=train_devices[0],
