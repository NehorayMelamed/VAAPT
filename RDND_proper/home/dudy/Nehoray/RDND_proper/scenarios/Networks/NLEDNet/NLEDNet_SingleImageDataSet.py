...
### Import Models: ###
from RDND_proper.models.NLEDN.lib.NLEDN import NLEDN
...
Train_dict.Network_checkpoint_prefix = 'NLEDNet'
...
Train_dict.network_free_flow_description = 'original training: mini-batch size of 1, adam optimizer, learning rate is initially set to 0.0005, and reduced b' \
                                                       'by 10% when the training loss stops decreasing, untill 0.0001. weight decay of 0.0001 and momentum of 0.9. ' \
                                                       'it takes around 3.5 days to train a model on DDN-Data or DIDMDN-data, and around 15hours to train on Rain100H. ' \
                                                       'for Rain100L dataset, it takes only about 4 hours.'
######################################################################################################################
...
### Load Network: ###
#######################
Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/NLEDN/weights/net_DDN.pth'
# Train_dict.load_Network_filename = r'/home/mafat/Pytorch_Checkpoints/Model_New_Checkpoints/NLEDNetNLEDNet_1/NLEDNetNLEDNet_1_TEST1_Step2000.tar'
######################################################################################################################
...
model = NLEDN(in_channels=3, out_channels=3)
...
preprocessing_object = PreProcessing_NLEDNet()
postprocessing_object = PostProcessing_NLEDNet()
#####################################################################################################################################################################
