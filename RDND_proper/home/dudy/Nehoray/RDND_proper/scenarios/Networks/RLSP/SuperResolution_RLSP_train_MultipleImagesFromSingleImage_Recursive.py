...
### Import Models: ###
from RDND_proper.models.RLSP.RLSP.pytorch.rlsp import *
...
### Paths: ###
#(1). General Paths:
project_path = path_fix_path_for_linux('/home/mmm/Desktop/tashtit/PROJECTS/rlsp')
inference_path_master = path_fix_path_for_linux("/home/mmm/Desktop/tashtit/PROJECTS/rlsp/Pytorch_Checkpoints")
TensorBoard_path = path_fix_path_for_linux("/home/mmm/Desktop/tashtit/PROJECTS/rlsp/Pytorch_Checkpoints\TensorBoard")
Network_checkpoint_folder = path_fix_path_for_linux("/home/mmm/Desktop/tashtit/PROJECTS/rlsp/Pytorch_Checkpoints\Model_New_Checkpoints")
#(2). Train Images:
Train_Images_Folder = path_fix_path_for_linux('/media/mmm/Elements/DataSets/Div2K/DIV2K_train_HR_BW')
# Train_Images_Folder = os.path.join(datasets_main_folder, '/DIV2K/Image_SuperResolution/RealSR (Final)/Nikon/Train/2')
#(3). Test Images
Test_Images_Folder = path_fix_path_for_linux('/media/mmm/Elements/DataSets/Div2K/DIV2K_train_HR_BW')
...
#Original time_steps_weights
Train_dict.time_steps_weights = [1,1,1,1,1] * 5
Train_dict.time_steps_weights[0]=0
Train_dict.time_steps_weights[-1]=0
...
TrainingCore.training_utils.set_seed(seed=1234)
######################################################################################################################
...
Train_dict.Network_checkpoint_prefix = 'RLSP_MultipleImages1Shift_Recursive_BatchSIze4OriginalLossLR1e3XInitScheduler1K'
...
Train_dict.network_free_flow_description = 'a fast recurrent network for super resolution, accepts 3 consecutive frames, ' \
                                                       'propagates internal states and does weight blending.'
######################################################################################################################
...
### Load Network: ###
#######################
Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/BasicSR/experiments/pretrained_models/EDVR/EDVR_L_deblur_REDS.tar'
######################################################################################################################
...
IO_dict.train_dataset_split_to_test_factor = 0.2
### Loading Images: ###
...
IO_dict_test.mean_gray_level_per_pixel = [200,200]
#######################################################################################################################################
...
number_of_model_input_frames=3
preprocessing_object = PreProcessing_RLSP_Recursive(number_of_model_input_frames)
preprocessing_object.flag_BW2RGB = True
postprocessing_object = PostProcessing_RLSP_Recursive()
postprocessing_object.number_of_model_input_frames = number_of_model_input_frames
...
optimization_dict.learning_rate_lowering_patience_before_lowering = 1000#20000
...
### Get Debug Callback: ###
debug_callback = InferenceCallback_SuperResolution_Recursive(inference_path + '_debug', Train_dict.is_training)  #TODO: isn't the Train_dict.is_training useless now?!!
debug_callback.frame_index_to_predict = 1
### Get Validation Callback: ###
validation_callback = InferenceCallback_SuperResolution_Recursive(inference_path + '_validation', Train_dict.is_training)
validation_callback.frame_index_to_predict = 1
