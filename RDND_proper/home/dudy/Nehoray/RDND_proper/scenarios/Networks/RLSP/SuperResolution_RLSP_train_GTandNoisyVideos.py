...
#(2). Train Images:
Train_Images_Folder = os.path.join(datasets_main_folder, '/DAVIS_CROPS/CropHeight_256_CropWidth_256')
#(3). Test Images
Test_Images_Folder = os.path.join(datasets_main_folder, '/DAVIS_CROPS/CropHeight_256_CropWidth_256')
...
Train_dict.Network_checkpoint_prefix = 'RLSP_MultipleImages1Shift_1'
...
IO_dict.number_of_image_frames_to_generate = 10
...
### Noise Parameters: ###
IO_dict.SNR = 200  # mean~10, std~2(readout)+sqrt(10)(shot)=5, effective_sigma~255/mean*5~127.5 -> SNR~4. this is only an approximation to better condition network
...
####################################
### DataSets: ###
train_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_GTandNoisyVideos(Train_Images_Folder, IO_dict)
test_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_GTandNoisyVideos(Test_Images_Folder, IO_dict_test)
...
preprocessing_object = PreProcessing_RLSP()
preprocessing_object.flag_BW2RGB = True
postprocessing_object = PostProcessing_RLSP()
...
Loss_object = TrainingCore.losses.Loss_Simple_RLSP(inputs_dict=None) #L1 only
...
### Get Debug Callback: ###
debug_callback = TrainingCore.callbacks.InferenceCallback_DenoisingSuperResolution_Base(inference_path + '_debug', Train_dict.is_training)  #TODO: isn't the Train_dict.is_training useless now?!!
### Get Validation Callback: ###
validation_callback = TrainingCore.callbacks.InferenceCallback_DenoisingSuperResolution_Base(inference_path + '_validation', Train_dict.is_training)
