...
TrainingCore.training_utils.set_seed(seed=4221)
...
model_path_icon= 'checkpoints/BasicVSR/IconVSR_REDS-aaa5367f.pth'
model_path_edvr = 'checkpoints/BasicVSR/EDVR_REDS_pretrained_for_IconVSR-f62a2f1e.pth'
...
#model = basicvsr_arch.IconVSR(num_feat=22, num_block=42,keyframe_stride=5,temporal_padding=2).to(device)

model.load_state_dict(torch.load(model_path_edvr)['params'])

len(torch.load(model_path_edvr)['params'])
len(torch.load(model_path_icon)['params'])
