...
TrainingCore.training_utils.set_seed(seed=122)
...
# x = outputs_dict['output_frames_original'][:,0,:,:,:]/255
...
#print()
print("diff is {} times".format((model_L_time)/(model_LW_time)))
...
# imshow_torch(x, False, 'original ')
# imshow_torch(x_sr_L[0], False, 'SwinIR model')
# imshow_torch(x_sr_LW[0], False, 'SwinIR Lightweight model')
#
#
# num_frames = IO_dict.number_of_image_frames_to_generate
...
# imshow_torch(x_sr_L[0][num_frames//2], False, 'SwinIR model')
# imshow_torch(x_sr_LW[0][num_frames//2], False, 'SwinIR Lightweight model')
# imshow_torch(x_sr_M[0][num_frames//2], False, 'SwinIR Medium model')
# imshow_torch(x_sr_div[0][num_frames//2], False, 'SwinIR div2k model')


num_frames = IO_dict.number_of_image_frames_to_generate
imshow_torch(x, False, 'Downsample')
imshow_torch(y, False, 'original')
imshow_torch((BW2RGB(RGB2BW(x_sr_L))), False, 'SwinIR model')
imshow_torch((BW2RGB(RGB2BW(x_sr_LW))), False, 'SwinIR Lightweight model')
imshow_torch((BW2RGB(RGB2BW(x_sr_M))), False, 'SwinIR Medium model')
imshow_torch((BW2RGB(RGB2BW(x_sr_div))), False, 'SwinIR div2k model')
...
# batch = torch.randn(1,3,64,64).cuda() # ~500Mb GPU Mem
# res = model(batch)