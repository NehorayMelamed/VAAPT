...
# Test_Images_Folder = os.path.join(datasets_main_folder, '/Vidmeo90K/vimeo_septuplet/sequences/00001')
# Test_Images_Folder = os.path.join(datasets_main_folder, '/GoPro/train')
...
Train_dict.Network_checkpoint_prefix = 'FastDVDNet_Deblur_1'
Train_dict.Network_checkpoint_prefix = 'NeuralGimbaless_Unet_1'
...
IO_dict.HR_string_pattern_to_search = '*sharp/*'
IO_dict.LR_string_pattern_to_search = '*blur/*'
...
IO_dict.number_of_images_per_video_to_load = 5
...
IO_dict.initial_crop_size = [128*1+5,128*1+5]
...
# train_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_SuperResolutionDeblur_DownSampleOnTheFly(Train_Images_Folder, IO_dict)
# test_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_SuperResolutionDeblur_DownSampleOnTheFly(Test_Images_Folder, IO_dict_test)
...
#####################################################################################################################################################################
### Model Itself: ###
# model = FastDVDnet_SuperResolution_NoNoiseMap(in_channels=1, num_input_frames=5)
# model = FastDVDnet_Deblur_NoNoiseMap_NoLimitOnFrames(in_channels=1, num_input_frames=5)
model = Unet_larger_NeuralGimbaless(in_channels=2, out_channels=(2-1)*2)  #output prediction per per image pair, each prediction is delta_x, delta_y, conf_x, conf_y
...
preprocessing_object = PreProcessing_NeuralGimbaless_Unet()
preprocessing_object.flag_BW2RGB = False
postprocessing_object = PostProcessing_NeuralGimbaless_Unet()
...
#####################################################################################################################################################################
### Get Loss Object: ###
Loss_object = TrainingCore.losses.Loss_Simple_NeuralGimbaless_Translation(inputs_dict=None) #L1 only
...
if Train_dict.flag_write_TB:
    ...
    TB_writer_test = None
TB_object_train = TB_SuperResolution(TB_writer_train)
TB_object_test = TB_SuperResolution(TB_writer_test)
