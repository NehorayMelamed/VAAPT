...
Original_Images_Folder = os.path.join(datasets_main_folder, '/Example Videos/LWIR_video_inf')
Inference_folder_name_addon = 'LWIR_3_onthefly'
###
...
### Noise Parameters: ###
IO_dict.sigma_to_dataset = 1/sqrt(3)*255
IO_dict.SNR_to_model = 3
...
IO_dict_original = EasyDict()
IO_dict_original.update(IO_dict)
IO_dict_original.sigma_to_dataset = 0
#######################################################################################################################################
...
test_dataset = TrainingCore.datasets.DataSet_SingleVideo_RollingIndex_AddNoise(root_folder=Test_Images_Folder,
                                                        transform_per_image=None,
                                                         ...
                                                         Train_dict=IO_dict)
original_dataset = TrainingCore.datasets.DataSet_SingleVideo_RollingIndex_AddNoise(root_folder=Original_Images_Folder,
                                                        transform_per_image=None,
                                                         transform_on_all_images=None,
                                                         image_loader=ImageLoaderCV,
                                                         number_of_images_per_video_to_scan=100,  #TODO: add possibility to randomize start index from a possibly long video
...
Train_dict.original_images_test_dataset = original_dataset
#####################################################################################################################################################################
...
debug_callback = TrainingCore.callbacks.InferenceCallback_Movie_ExtraDataset(inference_path, Train_dict.is_training)
# debug_callback = None
