...
#(2). Train Images:
# Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data/testCar'))
Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data/DIV2K_train_HR'))
# Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/Drones')
# Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/Thermal_Images_test')
# Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/words/a01/a01-000u/')
# Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'DIV2K_train_HR'))
# Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'ALPR/train/images'))
# Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'ALPR/check 1/images'))
# Train_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'ALPR/check_crop'))
# (3). Test Images
# Test_Images_Folder = os.path.join(datasets_main_folder, '/Vidmeo90K/vimeo_septuplet/sequences/00001/0021')
Test_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data/DIV2K_valid_HR'))
# Test_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/Drones')
# Test_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data/testCar'))
# Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/words/a01/a01-000u/')
# Test_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'DIV2K_valid_HR'))
# Test_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'ALPR/check 2/images'))
# Test_Images_Folder = path_fix_path_for_linux(os.path.join(base_path, 'Data', 'ALPR/test/images'))
#RGB images
...
Train_dict.Network_checkpoint_prefix = 'Restormer (single image defocus deblurring pth) Div2K [320,320] SR-Deblur'
...
copyfile(full_name, os.path.join(base_path,'Scripts',Train_dict.Network_checkpoint_prefix + '.py'))
...
IO_dict.flag_to_RAM = False #False
...
scale = 0.5 # To make crop_size changes easier
offset = 0
s1 = 8 * 3
s2 = 8 * 2
sz = [320,320] #width #height

IO_dict.initial_crop_size = sz
IO_dict.final_crop_size = sz
...
IO_dict_test.initial_crop_size = sz
IO_dict_test.final_crop_size = sz
#######################################################################################################################################
...
####################################
### DataSets: ###
train_dataset = TrainingCore.datasets.Dataset_MultipleImagesFromSingleImage_AWGNNoise_Shifts_SuperResolutionDeblur_DownSampleOnTheFly(Train_Images_Folder, IO_dict)
test_dataset = TrainingCore.datasets.Dataset_MultipleImagesFromSingleImage_AWGNNoise_Shifts_SuperResolutionDeblur_DownSampleOnTheFly(Test_Images_Folder, IO_dict_test)
...
### Get Batch: ###
batch_output_frames = outputs_dict['output_frames_original']
x = batch_output_frames/255
#x = x.squeeze(dim=2)

model = Restormer().cuda()
model_path_restormer = os.path.join(base_path,'Pretrained Checkpoints/Restormer/single_image_defocus_deblurring.pth')
model.load_state_dict(torch.load(model_path_restormer)['params'])
...
postprocessing_object.flag_BW2RGB = False
#####################################################################################################################################################################
