...
Train_dict.Network_checkpoint_prefix = 'FastDVDNet (pretrained) ALPR [400,256] (rannd noise)'
...
model_path = os.path.join(base_path,'Pretrained Checkpoints/FastDVDNet/model.pth') # standard deblurring (motion blur).
...
for key in model_dict:
    new_dict['.'.join(key.split('.')[1:])] = model_dict[key]
...
noise = torch.zeros(1,1,256,400).cuda()
with torch.no_grad():
    xx = model(x.cuda(),noise)
