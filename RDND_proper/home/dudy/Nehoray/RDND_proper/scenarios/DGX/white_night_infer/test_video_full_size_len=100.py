...
Train_Images_Folder = os.path.join(datasets_main_folder, 'Mitspe/test_video')
...
Test_Images_Folder = os.path.join(datasets_main_folder, 'Mitspe/test_video')
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + '100'
...
### Devices: ###
devices = [10,10]
IO_dict.test_device = torch.device(devices[0])
IO_dict.device = torch.device(devices[1])
...
IO_dict.test_args = TestArgs(tile=[20,128,128], nonblind_denoising=True)
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames_checkpoint(False, model_denoise_path, IO_dict.device, IO_dict.test_device)
