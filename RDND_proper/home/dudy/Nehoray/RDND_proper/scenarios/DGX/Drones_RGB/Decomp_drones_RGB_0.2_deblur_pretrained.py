...
### Import Models: ###
from KAIR_light.models.vrt_models import VRT_deblur_6_frames_8_GPUs
...
# model_denoise_path = osp.join(Network_checkpoint_folder, 'Denoise_drones_SBNUC_RGB.py/Denoise_drones_SBNUC_RGB.py_TEST1_Step975.tar')
model_deblur_path = osp.join(base_path, 'Checkpoints/VRT/007_VRT_videodeblurring_REDS.pth')
model_checkpoints = [model_deblur_path, model_deblur_path]

#(2). Train Images:
# Train_Images_Folder = osp.join(datasets_main_folder, 'REDS/train_blur')
# Train_Images_Folder = osp.join(base_path, 'save_folder')
Train_Images_Folder = osp.join(datasets_main_folder, 'drones_RGB_comp_0.2_0.2_2')
...
Test_Images_Folder = osp.join(datasets_main_folder, 'drones_RGB_comp_0.2_0.2_2')
...
IO_dict.Corrupted_string_pattern_to_search = '*comp*'# 'comp'
IO_dict.GT_string_pattern_to_search = '*clean*' #'clean'
...
IO_dict.PPP = 2.5 #[2, 10]  # Photons per Pixel.
### Noise Addition: ###
IO_dict.noise_dict = get_default_noise_dict()
IO_dict.noise_dict.RRNU_sigmas_polynomial_params = [1,1]
IO_dict.noise_dict.CRNU_sigmas_polynomial_params = [1,1]
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_deblur_6_frames_8_GPUs(True, model_deblur_path, IO_dict.model_devices)
