...
Train_Images_Folder = osp.join(base_path, 'Datasets/save_folder_thermal')
...
Test_Images_Folder = osp.join(base_path, 'Datasets/save_folder_thermal')
...
Train_dict.tensorboard_train_frequency = 125
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + '_Thermal_quant_PP=[2,10]'
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames_8_GPUs(pretrained=True, checkpoint_path=model_path, devices=IO_dict.model_devices)
