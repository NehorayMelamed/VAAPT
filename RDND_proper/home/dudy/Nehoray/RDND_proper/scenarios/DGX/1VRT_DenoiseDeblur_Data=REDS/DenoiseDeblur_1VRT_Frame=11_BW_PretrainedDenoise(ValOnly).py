#TODO: add scp back to comuter
# handle imshowtorch
# run without blur - ########
# run moving images with blur + denoise
# run vrt - 2/4 guys with more parts unfrozen
# run 2 vrts denoise + deblur
# get gpu, gou's based on what is running
...
Train_Images_Folder = osp.join(datasets_main_folder, 'REDS/val_sharp')
...
Train_dict.debug_step = 250
Train_dict.save_models_frequency = 250
Train_dict.validation_set_frequency = 250
Train_dict.tensorboard_train_frequency = 250
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + '_sharp'
...
### Devices: ###
IO_dict.device = torch.device(14)
...
IO_dict.train_dataset_split_to_test_factor = 0.35
### Loading Images: ###
#(1). Folder names:
#(1.1). Root Folder:
IO_dict.root_folder = ''
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_11_frames(pretrained=True, checkpoint_path=model_path).to(IO_dict.device)
