...
model_path = osp.join(base_path, 'Checkpoints/VRT/007_VRT_videodeblurring_REDS.pth')
...
Train_dict.save_models_frequency = 125
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + ' try_3'
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_deblur_6_frames_8_GPUs(pretrained=True, checkpoint_path=model_path, devices=IO_dict.model_devices)
