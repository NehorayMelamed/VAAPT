...
### Devices: ###
device_num = 11
IO_dict.device = torch.device(device_num)
print ('Starting to trian on GPUs: [', device_num, ']')
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames(pretrained=True, checkpoint_path=model_path).to(IO_dict.device)
