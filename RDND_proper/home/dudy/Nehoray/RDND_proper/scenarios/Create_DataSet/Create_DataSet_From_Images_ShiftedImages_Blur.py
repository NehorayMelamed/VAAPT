...
for max_shift_in_pixels in max_shifts_in_pixels:
    with_blur_shift_folder = os.path.join(super_blur_folder, 'Shift_' + str(max_shift_in_pixels))
    no_blur_shift_folder = os.path.join(super_shifts_folder, 'Shift_' + str(max_shift_in_pixels))
    ...
    imgaug_parameters['affine_translation_number_of_pixels'] = {"x": (-0, 0), "y": (-0, 0)}
    ...
    for image_index in arange(len(train_dataset)):
        ...
        for time_step in arange(number_of_time_steps):
            ...
            for PSNR_counter, PSNR in enumerate(additive_noise_PSNR):
                ...
                image_within_noise_folder_with_blur = os.path.join(noise_gain_folder_with_blur, string_rjust(image_index, 4))
                image_within_noise_folder_no_blur = os.path.join(noise_gain_folder_no_blur, string_rjust(image_index, 4))
                ...
                save_image_numpy(image_within_noise_folder_with_blur, str(time_step).rjust(4, '0') + '.png', numpy_array_with_blur, flag_convert_bgr2rgb=False, flag_scale=False)
                save_image_mat(image_within_noise_folder_with_blur, str(time_step).rjust(4,'0') + '.mat', current_frame_noisy_with_blur, 'center_frame_noisy')
