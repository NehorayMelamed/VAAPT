...
from .pwc_modules import OccContextNetwork, OccEstimatorDense
...
class PWCNet(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        for l, (x1_current_features, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l <= self.output_level:
                ...
                # correlation
                out_corr_f = Correlation(pad_size=self.search_range, kernel_size=1, max_displacement=self.search_range, stride1=1, stride2=1, corr_multiply=1)(x1_current_features, x2_warp)
                out_corr_b = Correlation(pad_size=self.search_range, kernel_size=1, max_displacement=self.search_range, stride1=1, stride2=1, corr_multiply=1)(x2, x1_current_features_warp)
                out_corr_relu_f = self.leakyRELU(out_corr_f)
                out_corr_relu_b = self.leakyRELU(out_corr_b)

                if l != self.output_level:
                    x1_current_features_1by1 = self.conv_1x1[l](x1_current_features)
                ...
                x_intm_f, flow_res_f = self.flow_estimators(torch.cat([out_corr_relu_f, x1_current_features_1by1, flow_f], dim=1))
                ...
                # occ estimation
                x_intm_occ_f, occ_res_f = self.occ_estimators(torch.cat([out_corr_relu_f, x1_current_features_1by1, occ_f], dim=1))
                ...
                occ_f = self.refine_occ(occ_cont_f.detach(), x1_current_features_1by1, x1_current_features_1by1 - x2_1by1_warp)
                occ_b = self.refine_occ(occ_cont_b.detach(), x2_1by1, x2_1by1 - x1_current_features_1by1_warp)
                ...
                occ_f = self.occ_shuffle_upsample(occ_f, torch.cat([x1_current_features_in, x2_w_in, flow_f, flow_b_warp], dim=1))
                occ_b = self.occ_shuffle_upsample(occ_b, torch.cat([x2_in, x1_current_features_w_in, flow_b, flow_f_warp], dim=1))
