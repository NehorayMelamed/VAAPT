...
class rdp_nuc(nn.Module):
    def __init__(self, in_channels=1, depth_rate=32, kernel_size=3, num_dense_layer=4, growth_rate=32):
    ...
    def forward(self, x):
        ...
        s2 = s2 + self.att0(s2)*self.bc0(s2)
        ...
        u2 = self.ups(a2*self.bc2(p2), size)
        u4 = self.ups(a4*self.bc4(p4), size)
        u8 = self.ups(a8*self.bc8(p8), size)
        u16 = self.ups(a16*self.bc16(p16), size)
