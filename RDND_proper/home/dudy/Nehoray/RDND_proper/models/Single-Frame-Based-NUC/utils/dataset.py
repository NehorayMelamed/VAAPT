...
def img2patch(vid, patch_size, patch_stride):
    # vid -> frame*hei*wid
...
def prepare_data(data_path, patch_size, patch_stride, frame_length, frame_stride, val_patch_size,
        ...
        for vid_name in vid_list:
            ...
            for s in range(len(scales)):
                ...
                for k in range(0, len(files) - frame_length, frame_stride):
                    ...
                    patches = img2patch(img, patch_size, patch_stride)
                    print('frame: %s->%s scale: %.1f samples %d' % (
                    k, k + frame_length - 1, scales[s], patches.shape[3] * aug_times))

                    for h in range(patches.shape[3]):
                        data = patches[:, :, :, h].copy()
    ...
    for vid_name in vid_list:
        ...
        for k in range(0, len(files) - val_frame_length, val_frame_stride):
            ...
            patches = img2patch(img, val_patch_size, val_patch_stride)
