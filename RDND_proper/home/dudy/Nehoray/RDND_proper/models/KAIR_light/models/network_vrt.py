...
class VRT(nn.Module):
    ...
    def _forward(self, x): #normal forward function
        ...
        if self.upscale == 1:
            ...
            x = x + self.conv_after_body(
                self.forward_features_deblur(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            ...
            x = self.conv_first(x.transpose(1, 2))
            x = x + self.conv_after_body(
                self.forward_features_sr(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
    ...
    def forward_features_sr(self, x, flows_backward, flows_forward):
    ...
    def forward_features_deblur(self, x, flows_backward, flows_forward):
        '''Main network for feature extraction.'''
...
class TestArgs():
    def __init__(self, tile, tile_overlap, scale, nonblind_denoising, window_size):
        self.tile = tile
