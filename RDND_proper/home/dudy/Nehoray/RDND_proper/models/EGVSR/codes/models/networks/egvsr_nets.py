...
from RDND_proper.models.EGVSR.codes.models.networks.base_nets import BaseSequenceGenerator, BaseSequenceDiscriminator
from RDND_proper.models.EGVSR.codes.utils.net_utils import space_to_depth, backward_warp, get_upsampling_func
from RDND_proper.models.EGVSR.codes.utils.data_utils import float32_to_uint8
...
class FRNet(BaseSequenceGenerator):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, degradation='BI', scale=4):
    ...
    def forward(self, lr_curr, lr_prev, hr_prev):
        ...
        lr_flow_pad = F.pad(lr_flow, (0, pad_w, 0, pad_h), 'zeros')
...
class FRNet_dudy(FRNet):
    """ Frame-recurrent network proposed in https://arxiv.org/abs/1801.04590
    """

    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, degradation='BI', scale=4):
        super(FRNet_dudy, self).__init__()

        self.scale = scale
        self.number_of_input_channels = in_nc
    ...
    def forward(self, x):
        ...
        pad_h = lr_curr.size(2) - lr_curr.size(2) // 8 * 8
        pad_w = lr_curr.size(3) - lr_curr.size(3) // 8 * 8
        lr_flow_pad = F.pad(lr_flow, (0, pad_w, 0, pad_h), 'circular')
...
from RapidBase.Utils.Array_Tensor_Manipulation import BW2RGB, BW2RGB_interleave_C, BW2RGB_interleave_T, RGB2BW, RGB2BW_interleave_C, RGB2BW_interleave_T
class FRNet_dudy_Jetson(FRNet):
    """ Frame-recurrent network proposed in https://arxiv.org/abs/1801.04590
    """
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=16, degradation='BI', scale=4):
        super(FRNet_dudy_Jetson, self).__init__()
    ...
    def forward(self, input_tensor):
        ...
        """
        ### Unpack incoming: ###
        lr_curr = input_tensor
        if self.lr_prev is None:
            self.lr_prev = BW2RGB(input_tensor)
        if self.hr_prev is None:
            self.hr_prev = BW2RGB(shuffle_up(torch.zeros_like(input_tensor).repeat(1, self.upsample_factor ** 2, 1, 1), self.upsample_factor).to(input_tensor.device))

        # estimate lr flow (lr_curr -> lr_prev)
        lr_flow = self.fnet(lr_curr, self.lr_prev)
        ...
        # warp hr_prev
        hr_prev_warp = backward_warp(self.hr_prev, hr_flow)
        ...
        ### Prepare optical flow for presentation: ###
        flow_magnitude = torch.sqrt(lr_flow_pad[:, 0:1] ** 2 + lr_flow_pad[:, 1:2] ** 2) #[B,1,H,W]

        return [hr_curr, lr_flow_pad]
