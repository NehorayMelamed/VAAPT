...
def get_uperleft_denominator(img, kernel):
    ker_f = convert_psf2otf(kernel, img.size()).to(img.device)  # discrete fourier transform of kernel
    nsr = wiener_filter_para(img).to(img.device)
    denominator = inv_fft_kernel_est(ker_f, nsr)#
    # img1 = img.cuda()
    # numerator = torch.rfft(img1, 3, onesided=False)
    # numerator = torch.view_as_real(torch.fft.fft(img, dim=3))
...
def inv_fft_kernel_est(ker_f, NSR):
    ...
    # imshow_torch_temp(inv_ker_f.abs())
    return inv_ker_f
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    # deblur_f = torch.zeros_like(inv_ker_f).to(inv_ker_f.device)
    ...
    # deblur = torch.fft.irfft(torch.view_as_complex(deblur_f), n=deblur_f.shape[3], dim=3)
    # # deblur = torch.fft.irfft(torch.view_as_complex(deblur_f), n=3, dim=-1)
    # # imshow_torch_temp(deblur)

    deblur_result = torch.fft.ifftn(inv_ker_f * fft_input_blur, dim=(-2, -1)).real
    # imshow_torch_temp(deblur_result)
...
def convert_psf2otf(ker, size):
    ...
    # otf = torch.view_as_real(torch.fft.fft(psf, dim=3))
