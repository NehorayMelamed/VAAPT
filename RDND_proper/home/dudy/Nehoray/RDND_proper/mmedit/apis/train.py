...
def _dist_train(model,
    ...
    if cfg.auto_resume:
        if cfg.resume_from:
            ...
            resume_path = find_resume_path(cfg)
            if resume_path:
                runner.resume(resume_path)
            else:
                print("[***] None resume file is founded !!")
                if cfg.load_from:
...
def find_resume_path(cfg):
    workdir = cfg.work_dir
    files = os.listdir(workdir)

    files = [x for x in files if x.endswith('.pth')]
    if len(files) == 0:
        return None
    else:
        iter_list = [int(x.split('.')[0].split('_')[-1]) for x in files]
        max_iter = np.max(iter_list)

        max_index = iter_list.index(max_iter)

        resume_path = os.path.join(workdir, files[max_index])
