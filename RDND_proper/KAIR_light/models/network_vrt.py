...
class VRT_6_frames(nn.Module):
    ...
    def forward(self, input_variable):
        x, input_dict = input_variable
        if input_dict.now_training:
            output = self._forward(x)
        else:
            output = self.test_video(x).to(input_dict.device)

        return output


    def _forward(self, x): #normal forward function
        ...
        if self.upscale == 1:
            ...
            x = x + self.conv_after_body(
                self.forward_features_deblur_6_frames(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            ...
            x = self.conv_first(x.transpose(1, 2))
            x = x + self.conv_after_body(
                self.forward_features_sr_6_frames(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
    ...
    def forward_features_sr_6_frames(self, x, flows_backward, flows_forward):
        ...
        total_iterations = 7
        no_grad_iterations = 4 # between 0 and 7
        with torch.no_grad(): #To save mem
            for idx in range(no_grad_iterations): # 7 Iterations
                layer = self.stage8[idx]
                x = layer(x)
        for idx in range(no_grad_iterations, total_iterations):  # 7 Iterations
            layer = self.stage8[idx]
            x = layer(x)

        # for layer in self.stage8: # 7 Iterations
        #     x = layer(x)
    ...
    def forward_features_deblur_6_frames(self, x, flows_backward, flows_forward):
        '''Main network for feature extraction.'''
        ...
        total_iterations = 5
        no_grad_iterations = 3  # between 0 and 5
        with torch.no_grad():  # To save mem
            for idx in range(no_grad_iterations):  # 5 Iterations
                layer = self.stage8[idx]
                x = layer(x)
        for idx in range(no_grad_iterations, total_iterations):  # 5 Iterations
            layer = self.stage8[idx]
            x = layer(x)

        # for layer in self.stage8: # 5 Iterations
        #     x = layer(x)
    ...
    def test_video(self, lq):
        ...
        scale = 1
        nonblind_denoising = self.nonblind_denoising
        window_size = [6, 8, 8]
        args = TestArgs(tile, tile_overlap, scale, nonblind_denoising, window_size)
        ...
        if num_frame_testing:
            ...
            for d_idx in d_idx_list:
                ...
                out_clip = self.test_clip(lq_clip, args)
        ...
        if size_patch_testing:
            ...
            for h_idx in h_idx_list:
                for w_idx in w_idx_list:
                    ...
                    out_patch = self._forward(in_patch).detach().cpu()
                    ...
                    w_idx * sf:(w_idx + size_patch_testing) * sf].add_(
                        out_patch)
            ...
            output = self._forward(lq).detach().cpu()
...
class VRT_11_frames(nn.Module):
    ...
    def _forward(self, x): #normal forward function
        ...
        if self.upscale == 1:
            ...
            x = self.conv_first(x.transpose(1, 2))
            x = x + self.conv_after_body(
                self.forward_features_deblur_11_frames(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            x = self.conv_last(x).transpose(1, 2)
            return x + x_lq
        else:
            # video sr
            x = self.conv_first(x.transpose(1, 2))
            x = x + self.conv_after_body(
                self.forward_features_sr_11_frames(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            x = self.conv_last(self.upsample(self.conv_before_upsample(x))).transpose(1, 2)
            _, _, C, H, W = x.shape
            return x + torch.nn.functional.interpolate(x_lq, size=(C, H, W), mode='trilinear', align_corners=False)
    ...
    def forward_features_sr_11_frames(self, x, flows_backward, flows_forward):
        '''Main network for feature extraction.'''
    ...
    def forward_features_deblur_11_frames(self, x, flows_backward, flows_forward):
        '''Main network for feature extraction.'''
        ...
        total_iterations = 5
        no_grad_iterations = 2  # between 0 and 5
        with torch.no_grad():  # To save mem
            for idx in range(no_grad_iterations):  # 5 Iterations
                layer = self.stage8[idx]
                x = layer(x)
...
class VRT_6_frames_8_GPUs(nn.Module):
    """ Video Restoration Transformer (VRT).
        A PyTorch impl of : `VRT: A Video Restoration Transformer`  -
          https://arxiv.org/pdf/2201.00000
    ...
    def __init__(self,
                 ...
                 no_checkpoint_attn_blocks=[],
                 no_checkpoint_ffn_blocks=[],
                 devices=[torch.device(i) for i in range (8)]):
        ...
        # conv_first
        self.conv_first = nn.Conv3d(in_chans*(1+2*4)+1 if self.nonblind_denoising else in_chans*(1+2*4),
                                    embed_dims[0], kernel_size=(1, 3, 3), padding=(0, 1, 1)).to(self.devices[1])

        # main body
        self.spynet = SpyNet(spynet_path, [2, 3, 4, 5]).to(self.devices[0])
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        reshapes = ['none', 'down', 'down', 'down', 'up', 'up', 'up']
        scales = [1, 2, 4, 8, 4, 2, 1]
        use_checkpoint_attns = [False if i in no_checkpoint_attn_blocks else use_checkpoint_attn for i in
        ...
        self.norm = norm_layer(embed_dims[-1]).to(self.devices[-1])
        self.conv_after_body = nn.Linear(embed_dims[-1], embed_dims[0]).to(self.devices[-1])

        # reconstruction
        num_feat = 64
        if self.upscale == 1:
            # for video deblurring, etc.
            self.conv_last = nn.Conv3d(embed_dims[0], in_chans, kernel_size=(1, 3, 3), padding=(0, 1, 1)).to(self.devices[-1])
        else:
            # for video sr
            self.conv_before_upsample = nn.Sequential(
                nn.Conv3d(embed_dims[0], num_feat, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
    ...
    def forward(self, input_variable):
        ...
        if input_dict.now_training:
            ...
            output = self.test_video(x, input_dict.test_args).to(input_dict.device)
    ...
    def _forward(self, x): #normal forward function
        ...
        flows_backward = [element.to(self.devices[1]) for element in flows_backward]
        flows_forward = [element.to(self.devices[1]) for element in flows_forward]
        # main network
        if self.upscale == 1:
            # video deblurring, etc.
            x = self.conv_first(x.transpose(1, 2))
            forwarded_features = self.conv_after_body(
                self.forward_features_deblur_6_frames(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            x = x.to(self.devices[-1])
            x = x + forwarded_features
            x = self.conv_last(x).transpose(1, 2).to(self.devices[0])
            return x + x_lq
        else:
            # video sr
            x = self.conv_first(x.transpose(1, 2))
    ...
    def forward_features_deblur_6_frames(self, x, flows_backward, flows_forward):
        ...
        x1, x2, x3 = x1.to(self.devices[2]), x2.to(self.devices[2]), x3.to(self.devices[2])
        flows_backward = [element.to(self.devices[2]) for element in flows_backward]
        flows_forward = [element.to(self.devices[2]) for element in flows_forward]

        x4 = self.stage4(x3, flows_backward[3::4], flows_forward[3::4])
        x = self.stage5(x4, flows_backward[2::4], flows_forward[2::4])
        x = self.stage6(x + x3, flows_backward[1::4], flows_forward[1::4])

        x, x1, x2 = x.to(self.devices[3]), x1.to(self.devices[3]), x2.to(self.devices[3])
        flows_backward = [element.to(self.devices[3]) for element in flows_backward]
        flows_forward = [element.to(self.devices[3]) for element in flows_forward]

        x = self.stage7(x + x2, flows_backward[0::4], flows_forward[0::4])
        x = x + x1

        for iter, layer in enumerate(self.stage8):
            x = layer(x)
            x = x.to(self.devices[min(iter+4,len(self.devices)-1)])
...
class VRT_denoise_deblur_6_frames_2_GPUs(nn.Module):

    def __init__(self, checkpoints=['', ''], devices=[torch.device(i) for i in range (2)]):
        super().__init__()
        self.devices = devices
        self.vrt_denoise = VRT_6_frames(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                         ...
                         nonblind_denoising=True).to(self.devices[0])
        self.vrt_denoise.load_state_dict(torch.load(checkpoints[0])['params'])

        self.vrt_deblur = VRT_6_frames(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                         depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                         indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                         num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16).to(
        self.devices[1])
        self.vrt_deblur.load_state_dict(torch.load(checkpoints[1])['params'])

    def forward(self, input_variable):
        x, input_dict = input_variable
        if input_dict.now_training:
            x = self.vrt_denoise._forward(x).to(self.devices[1])
            x = self.vrt_deblur._forward(x).to(self.devices[0])
        else:
            x = self.vrt_denoise.test_video(x).to(self.devices[1])
            x = self.vrt_deblur.test_video(x).to(self.devices[0])

        return x

class VRT_denoise_deblur_11_frames_2_GPUs(nn.Module):

    def __init__(self, checkpoints=['', ''], devices=[torch.device(i) for i in range (2)]):
        super().__init__()
        self.devices = devices
        self.vrt_denoise = VRT_11_frames(upscale=1, img_size=[6, 128, 128], window_size=[6, 8, 8],
                         depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                         indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                         num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
                         nonblind_denoising=True).to(self.devices[0])
        ...
        self.vrt_deblur = VRT_11_frames(upscale=1, img_size=[6, 128, 128], window_size=[6, 8, 8],
                         depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                         indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                         num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16).to(
        self.devices[1])
...
class VRT_denoise_deblur_6_frames_16_GPUs(nn.Module):

    def __init__(self, checkpoints=['', '']):
        super().__init__()
        self.devices = [torch.device(i) for i in range (16)]
        self.vrt_denoise = VRT_6_frames_8_GPUs(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                         depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                         indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                         num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
                         nonblind_denoising=True, devices = [torch.device(i) for i in range (8)])
        self.vrt_denoise.load_state_dict(torch.load(checkpoints[0])['params'])

        self.vrt_deblur = VRT_6_frames_8_GPUs(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                         depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                         indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                         num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16, devices = [torch.device(i) for i in range (8,16)])
        self.vrt_deblur.load_state_dict(torch.load(checkpoints[1])['params'])

    def forward(self, input_variable):
        x, input_dict = input_variable
        if input_dict.now_training:
            x = self.vrt_denoise._forward(x).to(self.devices[8])
            x = self.vrt_deblur._forward(x).to(self.devices[0])
        else:
            x = self.vrt_denoise.test_video(x).to(self.devices[8])
            x = self.vrt_deblur.test_video(x).to(self.devices[0])

        return x

class VRT_denoise_denoise_6_frames_16_GPUs(nn.Module):
    ...
    def __init__(self, checkpoints=['', '']):
        ...
        self.vrt_denoise = VRT_6_frames_8_GPUs(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                                               ...
                                               deformable_groups=16,
                                               nonblind_denoising=True, devices=[torch.device(i) for i in range(8,16)])
        self.vrt_denoise.load_state_dict(torch.load(checkpoints[1])['params'])
    ...
    def forward(self, input_variable):
        ...
        if input_dict.now_training:
            x = self.vrt_denoise._forward(x).to(self.devices[8])
            B, T, C, H, W = x.shape
            x = torch.cat(x, torch.zeros((B, T, 1, H, W)).to(self.devices[8]), 2)
            x = self.vrt_deblur._forward(x).to(self.devices[0])
        else:
            x = self.vrt_denoise.test_video(x).to(self.devices[8])
            x = torch.cat(x, torch.zeros((B, T, 1, H, W)).to(self.devices[8]), 2)
...
class VRT(nn.Module):
    ...
    def __init__(self,
                 ...
                 no_checkpoint_attn_blocks=[],
                 no_checkpoint_ffn_blocks=[],
                 train_device=torch.device(1),
                 test_device=torch.device(2),
                 test_args=TestArgs(),
                 denoise_prior=False,
                 pnp=False
                 ):
        ...
        self.denoise_prior = denoise_prior
        self.pnp = pnp
    ...
    def forward(self, input_variable):
        ...
        if input_dict.now_training:
            ...
            self.to(self.test_device)
            output = self.test_video(x).to(self.train_device)
    ...
    def _forward(self, x):
        ...
        # obtain noise level map
        if self.denoise_prior:
            if self.pnp:
                noise_level = 10 # change that please
                noise_level_map = pytorch_fastdvdnet_video_denoiser(x , self.dvdnet, noise_level, x.device, x.device).mean(axis=2, keepdim=True)
            # self.dvdnet =  self.dvdnet.to(self.train_device)
            else:
                noise_level_map = self.dvdnet(x)
        else:
            if self.nonblind_denoising:
                x, noise_level_map = x[:, :, :self.in_chans, :, :], x[:, :, self.in_chans:, :, :]
        ...
        if self.denoise_prior:
            if not self.pnp:
                noise_level_map = noise_level_map.unsqueeze(0).unsqueeze(2)
            x = torch.cat([x, noise_level_map], 2)
        else:
            if self.nonblind_denoising:
                x = torch.cat([x, noise_level_map], 2)
    ...
    def test_video(self, lq):
        ...
        if num_frame_testing:
            ...
            not_overlap_border = False
            b, d, c, h, w = lq.size()
            c = c - 1 if args.nonblind_denoising and (not self.denoise_prior) else c
            stride = num_frame_testing - num_frame_overlapping
            d_idx_list = list(range(0, d - num_frame_testing, stride)) + [max(0, d - num_frame_testing)]
            E = torch.zeros(b, d, c, h * sf, w * sf)
            W = torch.zeros(b, d, 1, 1, 1)
...
class VRT_denoise_deblur(nn.Module):

    def __init__(self, checkpoints=['', ''], train_devices=[torch.device(i) for i in range (2)],
        ...
        self.vrt_denoise = VRT(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8], depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
                nonblind_denoising=True, train_device=train_devices[0], test_device=test_devices[0], test_args=TestArgs()).to(train_devices[0])
        if pretrained[0]:
            self.vrt_denoise.load_state_dict(torch.load(checkpoints[0])['params'])

        else:  # Loading our checkpoint
            self.vrt_denoise.load_state_dict(torch.load(checkpoints[0], map_location="cuda:" + str(train_devices[0].index))['model_state_dict'])

        self.vrt_deblur = VRT(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8],
                               depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                               indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                               num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
                               nonblind_denoising=False, train_device=train_devices[1], test_device=test_devices[1], test_args=TestArgs(nonblind_denoising=False)).to(train_devices[1])
        if pretrained[1]:
            self.vrt_deblur.load_state_dict(torch.load(checkpoints[1])['params'])

        else:  # Loading our checkpoint
            self.vrt_deblur.load_state_dict(
                torch.load(checkpoints[1], map_location="cuda:" + str(train_devices[1].index))['model_state_dict'])

    def forward(self, input_variable):
        x, input_dict = input_variable
        x = self.vrt_denoise((x, input_dict)).to(self.train_devices[1])
        x = self.vrt_deblur((x, input_dict)).to(self.train_devices[0])
        return x

class VRT_denoise_SR(nn.Module):

    def __init__(self, checkpoints=['', ''], train_devices=[torch.device(i) for i in range (2)],
                 test_devices=[torch.device(i) for i in range (2,4)], pretrained = [False, False], freeze=[False,False], all_pretrained=False):
        super().__init__()
        self.train_devices = train_devices
        self.test_devices = test_devices
        self.freeze = freeze
        self.vrt_denoise = VRT(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8], depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
                indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
                num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
                nonblind_denoising=True, train_device=train_devices[0], test_device=test_devices[0], test_args=TestArgs(tile=[20,192,192], nonblind_denoising=True)).to(train_devices[0])
        ...
        if not all_pretrained:
            if pretrained[0]:
                ...
                self.vrt_denoise.load_state_dict(torch.load(checkpoints[0], map_location="cuda:" + str(train_devices[0]))['model_state_dict'])

        if self.freeze[0]:
            for param in self.vrt_denoise.parameters():
                param.requires_grad = False

        self.vrt_SR = VRT(upscale=4, img_size=[6, 160, 160], window_size=[6, 8, 8],
                                                  depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4, 4, 4],
                                                  indep_reconsts=[11, 12],
                                                  embed_dims=[120, 120, 120, 120, 120, 120, 120, 180, 180, 180, 180,
                                                              180, 180],
                                                  num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2,
                                                  deformable_groups=12,
                                                  nonblind_denoising=False, train_device=train_devices[1], test_device=test_devices[1], test_args=TestArgs(tile=[20,192,192], nonblind_denoising=False, scale=4)).to(train_devices[1])
        if not all_pretrained:

            if pretrained[1]:
                self.vrt_SR.load_state_dict(torch.load(checkpoints[1])['params'])

            else:  # Loading our checkpoint
                self.vrt_SR.load_state_dict(
                    torch.load(checkpoints[1], map_location="cuda:" + str(train_devices[1]))['model_state_dict'])

        if self.freeze[1]:
            for param in self.vrt_SR.parameters():
                param.requires_grad = False

        if all_pretrained:
            self.load_state_dict(torch.load(checkpoints[1], map_location="cuda:" + str(train_devices[1]))
                                 ['model_state_dict'])
    ...
    def forward(self, input_variable):
        ...
        x = self.vrt_denoise((x, input_dict)).to(self.train_devices[1])
        x = self.vrt_SR((x, input_dict)).to(self.train_devices[0])

        return x


class VRT_dudy(nn.Module):
    """ Video Restoration Transformer (VRT).
        A PyTorch impl of : `VRT: A Video Restoration Transformer`  -
          https://arxiv.org/pdf/2201.00000
    Args:
    ...
    def __init__(self,
        ...
        self.test_device = test_device
        self.test_args = test_args
        self.noises_flags = noises_flags
        ...
        # reconstruction for video denoising, etc.
        num_feat = 64
        self.conv_last = nn.Conv3d(96, in_chans, kernel_size=(1, 3, 3), padding=(0, 1, 1))
    ...
    def _forward(self, x):
        ...
        if self.upscale == 1:
            ...
            x = x + self.conv_after_body(
                self.forward_features(x, flows_backward, flows_forward).transpose(1, 4)).transpose(1, 4)
            x = self.conv_last(x).transpose(1, 2)
            x = self.get_corrections(x, x_lq)

            return x

    def get_corrections(self, x, x_lq):
        #add sigmoid,add epsilon, maybe add softmax,
        ### Being UBER-CHUCHEM: Inserting A Confidence Map: ###
        B, T, C, H, W = x.shape
        confidence_per_pixel = torch.sigmoid(x[:, :, 0:1])
        eps = 1e-3
        confidence_per_pixel_normalized_rows = confidence_per_pixel / (confidence_per_pixel.sum([-1], True) + eps)
        confidence_per_pixel_normalized_cols = confidence_per_pixel / (confidence_per_pixel.sum([-2], True) + eps)
        confidence_per_pixel_normalized_time = confidence_per_pixel / (confidence_per_pixel.sum([1], True) + eps)
        confidence_per_pixel_normalized_time_rows = confidence_per_pixel / (confidence_per_pixel.sum([1,4], True) + eps)
        confidence_per_pixel_normalized_time_cols = confidence_per_pixel / (confidence_per_pixel.sum([1,3], True) + eps)

        # # (*). NU part (with mean over time domain):

        # confidence_offset_per_row = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-1],True).repeat(1, 1, 1, 1, W)
        # confidence_offset_per_col = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-2],True).repeat(1, 1, 1, H, 1)
        # confidence_gain_per_row = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-1], True).repeat(1, 1, 1, 1, W)
        # confidence_gain_per_col = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-2], True).repeat(1, 1, 1, H, 1)


        # confidence_temporal_per_pixel = confidence_per_pixel
        # confidence_temporal_per_row = confidence_per_pixel.mean([-1], True).repeat(1, 1, 1, 1, W)
        # confidence_temporal_per_col = confidence_per_pixel.mean([-2], True).repeat(1, 1, 1, H, 1)
        #
        # # (*). NU part (with mean over time domain):
        # confidence_offset_per_pixel = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1)
        # confidence_offset_per_row = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-1],True).repeat(1, 1, 1, 1, W)
        # confidence_offset_per_col = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-2],True).repeat(1, 1, 1, H, 1)
        # confidence_gain_per_pixel = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1)
        # confidence_gain_per_row = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-1], True).repeat(1, 1, 1, 1, W)
        # confidence_gain_per_col = confidence_per_pixel.mean([1], True).repeat(1, T, 1, 1, 1).mean([-2], True).repeat(1, 1, 1, H, 1)

        ch = 1
        if self.noises_flags[0]:
            temporal_per_pixel = x[:, :, ch:ch + 1]
            ch += 1
        else:
            temporal_per_pixel = 0

        if self.noises_flags[1]:
            temporal_per_row = x[:, :, ch:ch + 1]
            # (2).
            if self.use_conf:
                temporal_per_row = (temporal_per_row * confidence_per_pixel_normalized_rows).sum([-1], True).repeat(1, 1, 1, 1, W)
            ch += 1
            # #(1).
            # nominator = (temporal_per_row * confidence_per_pixel).mean([-1], True).repeat(1, 1, 1, 1, W)
            # denominator = confidence_per_pixel.mean([-1], True).repeat(1, 1, 1, 1, W) + 1e-3
            # temporal_per_row = nominator / denominator

        else:
            temporal_per_row = 0

        if self.noises_flags[2]:
            temporal_per_col = x[:, :, ch:ch + 1]
            if self.use_conf:
                temporal_per_col = (temporal_per_col * confidence_per_pixel_normalized_cols).sum([-2], True).repeat(1, 1, 1, H, 1)
            ch += 1
        else:
            temporal_per_col = 0

        if self.noises_flags[3]:
            temporal_gain_per_pixel = x[:, :, ch:ch + 1]
            ch += 1
        else:
            temporal_gain_per_pixel = 0

        if self.noises_flags[4]:
            nuc_offset_per_pixel = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_offset_per_pixel = (nuc_offset_per_pixel * confidence_per_pixel_normalized_time).sum([1], True).repeat(1,T,1,1,1)
            ch += 1
        else:
            nuc_offset_per_pixel = 0

        if self.noises_flags[5]:
            nuc_gain_per_pixel = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_gain_per_pixel = (nuc_gain_per_pixel * confidence_per_pixel_normalized_time).sum([1], True).repeat(1,T,1,1,1)
            ch += 1
        else:
            nuc_gain_per_pixel = 0

        if self.noises_flags[6]:
            nuc_offset_per_row = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_offset_per_row = (nuc_offset_per_row * confidence_per_pixel_normalized_time_rows).sum([1,4], True).repeat(1, T, 1, 1,1).repeat(1, 1, 1, 1, W)
            ch += 1
        else:
            nuc_offset_per_row = 0

        if self.noises_flags[7]:
            nuc_gain_per_row = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_gain_per_row = (nuc_gain_per_row * confidence_per_pixel_normalized_time_rows).sum([1,4], True).repeat(1, T, 1, 1,1).repeat(1, 1, 1, 1, W)
            ch += 1
        else:
            nuc_gain_per_row = 0

        if self.noises_flags[8]:
            nuc_offset_per_col = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_offset_per_col = (nuc_offset_per_col * confidence_per_pixel_normalized_time_cols).sum([1,3], True).repeat(1, T, 1, 1,1).repeat(1, 1, 1, H, 1)
            ch += 1
        else:
            nuc_offset_per_col = 0

        if self.noises_flags[9]:
            nuc_gain_per_col = x[:, :, ch:ch + 1]
            if self.use_conf:
                nuc_gain_per_col = (nuc_gain_per_col * confidence_per_pixel_normalized_time_cols).sum([1,3], True).repeat(1, T, 1, 1,1).repeat(1, 1, 1, H, 1)
            ch += 1
        else:
            nuc_gain_per_col = 0

        ### Correct for offset NU: ###
        total_offset_nu = nuc_offset_per_pixel + nuc_offset_per_row + nuc_offset_per_col
        x = x_lq - total_offset_nu

        ### Correct for additive temporal noise: ###
        total_temporal_noise = temporal_per_pixel + temporal_per_row + temporal_per_col
        x = x + total_temporal_noise

        ### Correct for multiplicative temporal noise (speckles): ###
        total_temporal_multiplicative_noise = temporal_gain_per_pixel
        x = x * (1 + total_temporal_multiplicative_noise)

        ### Correct for gain NU: ###
        total_gain_nu = nuc_gain_per_pixel + nuc_gain_per_row + nuc_gain_per_col
        x = x * (1 + total_gain_nu)
...
#
# model = VRT(upscale=1, img_size=[11, 128,128], window_size=[6, 8, 8], depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
#                 indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
#                 num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
#                 nonblind_denoising=True,
#             use_checkpoint_attn=True,
#             use_checkpoint_ffn=True).to(torch.device(6))
# # print(model)
# print('{:>16s} : {:<.4f} [M]'.format('#Params', sum(map(lambda x: x.numel(), model.parameters())) / 10 ** 6))
# #
# x = torch.randn((1, 11, 4, 128,128)).to(torch.device(6))
# y = model(x)
# devices = [torch.device(i) for i in range (8,16)]
# x = torch.randn(1,6,4,192,192).to(devices[0])
# net = VRT_6_frames_8_GPUs(upscale=1, img_size=[6, 192, 192], window_size=[6, 8, 8], depths=[8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4],
#                 indep_reconsts=[9, 10], embed_dims=[96, 96, 96, 96, 96, 96, 96, 120, 120, 120, 120],
#                 num_heads=[6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6], pa_frames=2, deformable_groups=16,
