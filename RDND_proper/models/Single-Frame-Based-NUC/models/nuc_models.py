...
from RDND_proper.models.attention_block import ChannelGate, SpatialGate
...
class NLSPP(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv_out = nn.Conv2d(5*in_channels, in_channels, kernel_size=1, padding=0)

        self.conv0 = basicConv(in_channels)
        ...
        self.catt = ChannelGate(16, 16, reduction_ratio=4, pool_types=['avg'])
        self.aatt1 = SpatialGate()
        self.aatt2 = SpatialGate()
    ...
    def forward(self, x):
        ...
        s   = torch.cat([x2, p2, p4, p8, p16], dim=1)
        out = self.conv_out(s)
        return out
...
class rdp_nuc(nn.Module):
    def __init__(self, in_channels=1, depth_rate=16, kernel_size=3, num_dense_layer=4, growth_rate=16):
        ...
        self.gff_1x1 = nn.Conv2d(depth_rate*4, depth_rate, kernel_size=1, padding=0)
        ...
        self.attw = basicAttention(5*depth_rate, 1)
        ...
        self.rk = nn.Conv2d(depth_rate*5, 1, kernel_size=1, padding=0)
        self.rb = nn.Conv2d(depth_rate*5, depth_rate, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        d3 = self.db3(d2)
        d4 = self.db4(d3)
        dc = torch.cat([d1, d2, d3, d4], dim=1)
        s2 = self.gff_1x1(dc)
        ...
        a8 = self.att8(p8)
        a16 = self.att16(p16)
        # w = self.attw(torch.cat([s2, self.ups(p2, size), self.ups(p4, size),
        #                          self.ups(p8, size), self.ups(p16, size)], dim=1))

        # --- Pyramid convolution block --- #
        u0 = self.ups(self.bc0(a0*s2)+a0*s2, size)
        u2 = self.ups(self.bc2(a2*p2)+a2*p2, size)
        u4 = self.ups(self.bc4(a4*p4)+a4*p4, size)
        u8 = self.ups(self.bc8(a8*p8)+a8*p8, size)
        u16 = self.ups(self.bc16(a16*p16)+a16*p16, size)
        uu = self.rb(torch.cat([u0, u2, u4, u8, u16], dim=1))

        # --- Weighted Linear parameters regression block --- #
        # k = self.rk(torch.cat([u0, u2, u4, u8, u16], dim=1))
        # b = self.rb(torch.cat([u0, u2, u4, u8, u16], dim=1))
        # out = (w*k+1-w)*x + b

        # b = self.rb(torch.cat([u0, u2, u4, u8, u16], dim=1))
        # out = x + b
        out = self.conv_out(uu)
        return out
