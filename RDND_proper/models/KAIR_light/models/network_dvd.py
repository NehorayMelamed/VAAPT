...
class FastDVDnet_Omer(nn.Module):
	...
	def forward(self, x):
		...
		# Unpack inputs
		x, noise_map = x[:, :, :C, :, :], x[:, :, C:, :, :]
		(x0, x1, x2, x3, x4) = tuple(x[:, m, :, :, :] for m in range(num_frames))
...
# C = 3
# b = 1
# x = torch.randn(b, num_frames, C+1, w, h)
#
# x, noise_level_map = x[:, :, :C, :, :], x[:, :, C:, :, :]
#
# (x0, x1, x2, x3, x4) = tuple(x[:, m, :, :, :] for m in range(num_frames))
# noise_level_map = noise_level_map[:, 0, :, :, :]
#
# y = torch.cat([x, noise_level_map], 2)