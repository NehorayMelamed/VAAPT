...
class PatchEmbed3Frames(nn.Module):
    def __init__(self, patch_size=16, in_chans=1, embed_dim=64, pe='linear'):
        super().__init__()
        self.patch_size = patch_size
        self.dim = embed_dim
...
class CostPerceiverEncoderNFrames(nn.Module):
    def __init__(self, cfg, N):
        super(CostPerceiverEncoderNFrames, self).__init__()
        self.cfg = cfg
        self.N = N
        self.patch_size = cfg.patch_size
        self.patch_embed = PatchEmbed(in_chans=self.N-1, patch_size=self.patch_size, embed_dim=cfg.cost_latent_input_dim, pe=cfg.pe)
        ...
        else: # we need to figure what to pass it
            self.vertical_encoder_layers = nn.ModuleList(
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        cost_maps = cost_volume.permute(0, 2, 3, 1, 4, 5).contiguous().view(B * H1 * W1, heads, H2, W2)
        ###
        # I want a function to take a cost map from [var, 2, h, w] to [var, 1, h, w],
        # maybe use attention, but for now lets be morons and use mean
        # note : changed after the patch embed so we can still use every thing in the conv of embedding
        ###
        ...
        data['cost_maps'] = data['cost_maps'].mean(1).unsqueeze(1)
        return x


class CostPerceiverEncoder3Frames(nn.Module):
    def __init__(self, cfg):
        super(CostPerceiverEncoder3Frames, self).__init__()
        self.cfg = cfg
        self.patch_size = cfg.patch_size
        self.patch_embed = PatchEmbed(in_chans=3, patch_size=self.patch_size, embed_dim=cfg.cost_latent_input_dim, pe=cfg.pe)
    ...
    def forward(self, cost_volume, data, context=None):
        # heads = 3
        # self.cfg.cost_heads_num should be 3
        B, heads, H1, W1, H2, W2 = cost_volume.shape
        cost_maps = cost_volume.permute(0, 2, 3, 1, 4, 5).contiguous().view(B * H1 * W1, heads, H2, W2)
        data['cost_maps'] = cost_maps
...
class MemoryEncoder3Frames(nn.Module):
    def __init__(self, cfg):
        super(MemoryEncoder3Frames, self).__init__()
        self.cfg = cfg
        ...
        if cfg.fnet == 'twins':
            ...
            exit()
        self.channel_convertor = nn.Conv2d(cfg.encoder_latent_dim, cfg.encoder_latent_dim, 1, padding=0, bias=False)
        self.cost_perceiver_encoder = CostPerceiverEncoder3Frames(cfg)
    ...
    def corr(self, fmap1, fmap2):
        ...
        fmap1 = rearrange(fmap1, 'b (heads d) h w -> b heads (h w) d', heads=self.cfg.cost_heads_num)
        fmap2 = rearrange(fmap2, 'b (heads d) h w -> b heads (h w) d', heads=self.cfg.cost_heads_num)
        # fmap3 = rearrange(fmap3, 'b (heads d) h w -> b heads (h w) d', heads=self.cfg.cost_heads_num)
        corr = einsum('bhid, bhjd -> bhij', fmap1, fmap2)
        corr = corr.permute(0, 2, 1, 3).view(batch*ht*wd, self.cfg.cost_heads_num, ht, wd)
        #corr = self.norm(self.relu(corr))
        corr = corr.view(batch, ht*wd, self.cfg.cost_heads_num, ht*wd).permute(0, 2, 1, 3)
    ...
    def forward(self, image1, image2, image3, data, context=None):
        feat_s = self.feat_encoder(image1)
        feat_t = self.feat_encoder(image2)
        feat_r = self.feat_encoder(image3)

        feat_s = self.channel_convertor(feat_s)
        feat_t = self.channel_convertor(feat_t)
        feat_r = self.channel_convertor(feat_r)
        ...
        if self.cfg.feat_cross_attn:
            feat_s = feat_s.flatten(2).transpose(1, 2)
            feat_t = feat_t.flatten(2).transpose(1, 2)
            feat_r = feat_r.flatten(2).transpose(1, 2)

            for layer in self.layers:
                feat_s, feat_t = layer(feat_s, feat_t, size)
                feat_t, feat_r = layer(feat_t, feat_r, size)
                # feat_s, feat_r = layer(feat_s, feat_t, size)

            feat_s = feat_s.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()
            feat_t = feat_t.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()
            feat_r = feat_r.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()

        cost_volume1 = self.corr(feat_s, feat_t)
        cost_volume2 = self.corr(feat_t, feat_r)
        cost_volume3 = self.corr(feat_s, feat_r)
        # (1, 3, h, w, h, w)
        x = self.cost_perceiver_encoder(torch.cat([cost_volume1, cost_volume2, cost_volume3], dim=1), data, context)
...
class MemoryEncoderNFrames(nn.Module):
    def __init__(self, cfg, N=3):
        super(MemoryEncoderNFrames, self).__init__()
        self.cfg = cfg
        self.N = N
        ...
        if cfg.fnet == 'twins':
            ...
            exit()
        self.channel_convertor = nn.Conv2d(cfg.encoder_latent_dim, cfg.encoder_latent_dim, 1, padding=0, bias=False)
        self.cost_perceiver_encoder = CostPerceiverEncoderNFrames(cfg, self.N)
    ...
    def forward(self, images, data, context=None):
        features = []
        for image in images[0]:
            feat = self.feat_encoder(image.unsqueeze(0))
            feat = self.channel_convertor(feat)
        ...
        if self.cfg.feat_cross_attn:
            for id, f in enumerate(features):
                features[id] = f.flatten(2).transpose(1, 2)

            for layer in self.layers:
                for id, f1, f2 in enumerate(zip(features, features[1:])):
                    features[id], features[id+1] =  layer(f1, f2, size)

            for id, f in enumerate(features):
                features[id] = f.reshape(B, *size, -1).permute(0, 3, 1, 2).contiguous()

        cost_volumes = []
        for id, [f1, f2] in enumerate(zip(features, features[1:])):
            cost_volume = self.corr(f1, f2)
            cost_volumes.append(cost_volume)

        # TODO: what to do about cost volume
        cost_volumes = torch.cat([cv for cv in cost_volumes], dim=1)
        # (1, 3, h, w, h, w)
        x = self.cost_perceiver_encoder(cost_volumes, data, context)
