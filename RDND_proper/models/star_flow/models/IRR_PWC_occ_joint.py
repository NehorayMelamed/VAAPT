...
class PWCNet(nn.Module):
    def __init__(self, args, div_flow=0.05):
        ...
        self.flow_and_occ_estimators = FlowAndOccEstimatorDense(self.num_ch_in)
    ...
    def forward(self, input_dict):
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if l <= self.output_level:
                ...
                # concat and estimate flow and occ
                ...
                x_intm_f, flow_res_f, occ_res_f = self.flow_and_occ_estimators(torch.cat([out_corr_relu_f, x1_1by1, flow_f, occ_f], dim=1))
                x_intm_b, flow_res_b, occ_res_b = self.flow_and_occ_estimators(torch.cat([out_corr_relu_b, x2_1by1, flow_b, occ_b], dim=1))
                flow_est_f = flow_f + flow_res_f
                flow_est_b = flow_b + flow_res_b
                occ_est_f = occ_f + occ_res_f
                occ_est_b = occ_b + occ_res_b

                flow_fine_f, occ_fine_f = self.context_networks(torch.cat([x_intm_f, flow_est_f, occ_est_f], dim=1))
                flow_fine_b, occ_fine_b = self.context_networks(torch.cat([x_intm_b, flow_est_b, occ_est_b], dim=1))

                flow_cont_f = flow_est_f + flow_fine_f
                flow_cont_b = flow_est_b + flow_fine_b
                occ_cont_f = occ_est_f + occ_fine_f
                occ_cont_b = occ_est_b + occ_fine_b
