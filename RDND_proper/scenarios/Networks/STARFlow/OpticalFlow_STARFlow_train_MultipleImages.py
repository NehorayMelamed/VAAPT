...
Train_dict.Network_checkpoint_prefix = 'STARFlow_MultipleImages_Translation_1'
...
Train_dict.network_free_flow_description = 'STARFlow'
######################################################################################################################
...
### Noise Parameters: ###
IO_dict.SNR = 3*255  # mean~10, std~2(readout)+sqrt(10)(shot)=5, effective_sigma~255/mean*5~127.5 -> SNR~4. this is only an approximation to better condition network
...
preprocessing_object = PreProcessing_STARFlow()
preprocessing_object.flag_BW2RGB = True
postprocessing_object = PostProcessing_STARFlow_OpticalFlow()
...
#####################################################################################################################################################################
### Get Loss Object: ###
Loss_object = TrainingCore.losses.Loss_Simple_RAFT_OpticalFlow(inputs_dict=None) #L1 only
...
### Get Debug Callback: ###
debug_callback = TrainingCore.callbacks.InferenceCallback_OpticalFlow_Base(inference_path + '_debug', Train_dict.is_training, Train_dict)  #TODO: isn't the Train_dict.is_training useless now?!!
### Get Validation Callback: ###
validation_callback = TrainingCore.callbacks.InferenceCallback_OpticalFlow_Base(inference_path + '_validation', Train_dict.is_training, Train_dict)
