...
Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/REDS/val_sharp/val/val_sharp/004')
# Train_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/test')
...
Test_Images_Folder = path_fix_path_for_linux('C:/Users/Omer Leibovitch/Desktop/Work/Data/REDS/val_sharp/val/val_sharp/004')
...
scale = 2 ** (-2) # To make crop_size changes easier
offset = 5
IO_dict.initial_crop_size = [int(128*scale)+offset, int(128*scale)+offset]
IO_dict.final_crop_size = [int(128*scale), int(128*scale)]

### Test/Validation Dict: ###
IO_dict_test = EasyDict(IO_dict)
IO_dict_test.flag_crop_mode = 'center'
IO_dict_test.initial_crop_size = [int(128*scale)+offset, int(128*scale)+offset] #np.inf
IO_dict_test.final_crop_size = [int(128*scale), int(128*scale)] #np.inf
...
### Test Models: ###
with torch.no_grad():
    x_compressed_deblur = model_edvr_compressed_deblur(x_upsampled_bicubic.cuda()) # RuntimeError: Unable to find a valid cuDNN algorithm to run convolution -
                                                                                    # VRAM memory limit was hit (which is rather non-intuitive from the error message).
...
preprocessing_object = PreProcessing_EDVR_deblur()
...
#####################################################################################################################################################################
### Optimizer: ###
Network_Optimizer_object = Optimizer_Base(model_edvr_compressed_deblur,
                                          intial_lr=1e-4,
                                          hypergrad_lr=0)
...
Trainer = GeneralTrainer(model=model_edvr_compressed_deblur,
