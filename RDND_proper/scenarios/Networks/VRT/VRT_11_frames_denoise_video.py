...
### Import Models: ###
from KAIR_light.models.vrt_models import VRT_denoise, VRT_denoise_11_frames
...
base_path = path_fix_path_for_linux('/home/mafat/')
...
datasets_main_folder = osp.join(base_path, 'Datasets')

model_path = osp.join(base_path, 'Checkpoints/VRT/008_VRT_videodenoising_DAVIS.pth')
#(2). Train Images:


Train_Images_Folder = osp.join(datasets_main_folder, 'REDS4')
#Images
Test_Images_Folder = osp.join(datasets_main_folder, 'REDS4')
...
Train_Images_Folder = os.path.join(datasets_main_folder, 'Example Videos/LWIR_video_original')
...
Train_dict.Network_checkpoint_prefix = 'VRT_11_frames_denoise_video (REDS)'
...
IO_dict.PPP = [15, 15]  # Photons per Pixel.
### Noise Addition: ###
...
IO_dict.col_readout_noise_sigma = 5
...
IO_dict.transforms_dict.blur_fraction = 0.5
...
# test_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_AGN(Test_Images_Folder, IO_dict_test)
...
# bla = train_dataset[4]
# video_torch_array_to_video((bla.output_frames_noisy/30).clip(0,1), os.path.join(datasets_main_folder, 'blibli.avi'), 2)
...
### Model Itself: ###
### Model Itself: ###
model = VRT_denoise_11_frames(pretrained=True, checkpoint_path=model_path)
