...
Train_Images_Folder = osp.join(datasets_main_folder ,'DIV2K_valid_HR')
#Images
Test_Images_Folder = osp.join(datasets_main_folder ,'DIV2K_valid_HR')
...
Train_dict.tensorboard_train_frequency = 50
...
Train_dict.Network_checkpoint_prefix = 'VRT_denoise_shot_noise_quant_11_frames_BW'
...
IO_dict.PPP = 1.5  # Photons per Pixel.
### Noise Addition: ###
...
IO_dict.transforms_dict.number_of_blur_steps_per_pixel = 0 #5
### Super Resolution: ###
...
# imagesc_torch(n)
#
# x = torch.randn(1,11,4,128,128).cuda()
# model = model.cuda()
# with torch.no_grad():
#     y = model(x,IO_dict)
