...
#(2). Train Images:
#Train_Images_Folder = os.path.join(datasets_main_folder, '/Vidmeo90K/vimeo_septuplet/sequences/00001/0021')
...
Test_Images_Folder = os.path.join(datasets_main_folder, '/DIV2K/DIV2K/DIV2K_train_HR')  #RGB images
...
preprocessing_object = PreProcessing_FastDVDNet()
...
### Get Debug Callback: ###
debug_callback = InferenceCallback_DenoisingSuperResolution_Base(inference_path + '_debug', Train_dict.is_training, Train_dict)  #TODO: isn't the Train_dict.is_training useless now?!!
debug_callback.downsampling_factor = 4
### Get Validation Callback: ###
validation_callback = InferenceCallback_DenoisingSuperResolution_Base(inference_path + '_validation', Train_dict.is_training, Train_dict)
validation_callback.downsampling_factor = 4
### Get Inference Callback: ###
inference_callback = validation_callback
inference_callback.downsampling_factor = 4
#####################################################################################################################################################################
...
batch_orig = a['center_frame_original']
first_orig = batch_orig[0]

imshow_torch(first_orig/255)

x = a['output_frames_original'].to(device)
xx = model(x)
xxx = xx[0][2]
