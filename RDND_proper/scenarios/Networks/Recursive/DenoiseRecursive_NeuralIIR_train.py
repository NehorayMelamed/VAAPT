...
### Import Models: ###
from RDND_proper.models.Classic_IIR.Classic_IIR import ClassicIIR_AlphaBlend_SimpleNetworkNoOpticalFlow_NoClassicalPrior
...
Train_Images_Folder = os.path.join(datasets_main_folder, '/DIV2K/DIV2K/Single_Image')
...
### Time Parametrs: ###
Train_dict.number_of_time_steps_in_one_backward = 3 #number of time steps before doing a .backward()
...
Train_dict.Network_checkpoint_prefix = 'NeuralIIR_1'
...
### Noise Parameters: ###
IO_dict.SNR = 50  # mean~10, std~2(readout)+sqrt(10)(shot)=5, effective_sigma~255/mean*5~127.5 -> SNR~4. this is only an approximation to better condition network
...
IO_dict.initial_crop_size = [128*4, 128*4]
...
# train_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_AddNoise(Train_Images_Folder, IO_dict)
# test_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_AddNoise(Test_Images_Folder, IO_dict_test)
# train_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_GTandNoisyVideos(Train_Images_Folder, None, IO_dict)
# test_dataset = TrainingCore.datasets.DataSet_Videos_In_Folders_GTandNoisyVideos(Test_Images_Folder, None, IO_dict_test)
...
### Model Itself: ###
from RDND_proper.models.Classic_IIR.Classic_IIR import *
# model = ClassicIIR_AlphaBlend_NoOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
# model = ClassicIIR_ResetGate_NoOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
# model = ClassicIIR_AlphaBlend_WithOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
# model = ClassicIIR_ResetGate_WithOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
model = ClassicIIR_AlphaBlend_SimpleNetworkNoOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
# model = ClassicIIR_ResetGate_SimpleNetworkNoOpticalFlow_NoClassicalPrior(model_dict=IO_dict)
...
preprocessing_object = PreProcessing_NeuralIIR()
preprocessing_object.flag_RGB2BW = False
preprocessing_object.flag_BW2RGB = False
postprocessing_object = PostProcessing_NeuralIIR()
...
### Get Debug Callback: ###
debug_callback = InferenceCallback_Denoise_Recursive_ClassicIIR(inference_path + '_debug', Train_dict)
debug_callback.frame_index_to_predict = 0
### Get Validation Callback: ###
validation_callback = InferenceCallback_Denoise_Recursive_ClassicIIR(inference_path + '_validation', Train_dict)
