...
from RDND_proper.models.MIRNet.networks.MIRNet_model import MIRNet
...
# Test_Images_Folder = path_fix_path_for_linux('/hoame/mafat/DataSets/DataSets\DIV2K\Official_Test_Images\Original_Images')  #Official & Interesting Test Images
...
Train_dict.network_free_flow_description = 'the models are trained with adam optimizer, beta1=0.9, beta2=0.999 for 7e5 iterations. ' \
                                                       'initial learning rate is 2e-4. we employ cosine annealing strategy to steadily decrease the learning' \
                                                       'rate from initial value to 1e-6 during training. training has patches of 128X128, batch size is 16. ' \
                                                       'for data augmentation we perform horizontal and vertical flips. ' \
                                                       'ablation studies show that SKFF (selective kernel feature fusion) uses about 6x fewer parameters than concateanation. increasing the ' \
                                                       'number of streams provides significant improvements, thereby justifying the importance of multi-scale features ' \
                                                       'processing. '
######################################################################################################################
...
### Load Network: ###
#######################
Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/MIRNet/pretrained_models/denoising/model_denoising.pth'
# Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/MIRNet/pretrained_models/enhancement/model_fivek.pth'
# Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/MIRNet/pretrained_models/enhancement/model_lol.pth'
# Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/MIRNet/pretrained_models/super_resolution/model_SR_x3.pth'
# Train_dict.load_Network_filename = r'/home/mafat/PycharmProjects/IMOD/models/MIRNet/pretrained_models/super_resolution/model_SR_x4.pth'
######################################################################################################################
...
####################################
### DataSets: ###
train_dataset = TrainingCore.datasets.Dataset_MultipleImagesFromSingleImage_AWGNNoise_BlurLowPass(Train_Images_Folder, IO_dict)
test_dataset = TrainingCore.datasets.Dataset_MultipleImagesFromSingleImage_AWGNNoise_BlurLowPass(Test_Images_Folder, IO_dict_test)
...
model = MIRNet(in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False)
...
preprocessing_object = PreProcessing_MIRNet()
postprocessing_object = PostProcessing_MIRNet()
#####################################################################################################################################################################
