...
# model_sr_path = '/raid/Pytorch_Checkpoints/2022-06-12/Model_New_Checkpoints/drones_SR[160,160].py/drones_SR[160,160].py_TEST1_Step16425.tar'
model_sr_path = '/raid/Pytorch_Checkpoints/2022-06-12/Model_New_Checkpoints/drones_SR_noised.py/drones_SR_noised.py_TEST1_Step16425.tar'
model_checkpoints = [model_denoise_path, model_deblur_path]

#(2). Train Images:
# Train_Images_Folder = osp.join(datasets_main_folder, 'binning_datasets/VisDroneX8')
...
# Test_Images_Folder = osp.join(datasets_main_folder, 'binning_datasets/VisDroneX8')
...
IO_dict.PPP = [4]  # Photons per Pixel.
### Noise Addition: ###
temp_factor = 30/255
...
IO_dict.noise_dict.flag_add_row_NU = True
IO_dict.noise_dict.flag_add_col_NU = True
...
IO_dict.flag_add_row_readout_noise = True
IO_dict.flag_add_col_readout_noise = True
...
w,h,offset =  192,192,0 #176,176,0# 12,512,0 128,128
...
w,h,offset = 512,512,0 #1080,1920, 0# 176,320,0# 12,512,0 128, 240
...
IO_dict_test.number_of_image_frames_to_generate = 18
IO_dict_test.number_of_images_per_video_to_load = 18
#######################################################################################################################################
...
######################################################################################################################################
# # ### Show images: ###
# downsampling_layer = nn.AvgPool2d(6)
# downsampling_layer2 = nn.AvgPool2d(5)
#
# bla = train_dataset[0]
...
# imshow_torch_video_seamless(a/a.max(), print_info=True)
# imshow_torch_video_seamless(b/b.max(), print_info=True)
# c = 2

# b = bla.output_frames_original
# bd = downsampling_layer(b)
# bdd = downsampling_layer2(b)
# # imshow_torch(a[0]/255)
# imshow_torch(b[0]/255)
# imshow_torch(bd[0]/255)
# imshow_torch(bdd[0]/255)
# plt.show(block=True)
#####################################################################################################################################################################
### Model Itself: ###
