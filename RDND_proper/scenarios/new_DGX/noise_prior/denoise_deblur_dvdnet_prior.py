...
shutil.copyfile(__file__, osp.join(script_path, Train_dict.Network_checkpoint_prefix +'.py'))
##########################################################################################
### IO: ###
...
IO_dict.test_args = TestArgs(tile=[20,192,192], nonblind_denoising=True)
...
model = VRT_denoise_6_frames_checkpoint(True, checkpoint_path=model_denoise_path, train_device=train_devices[0],
                                       test_device=test_devices[0], test_args=IO_dict.test_args, denoise_prior=True)
