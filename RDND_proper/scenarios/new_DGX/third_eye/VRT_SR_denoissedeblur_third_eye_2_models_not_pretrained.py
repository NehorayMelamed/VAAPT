...
### Import Models: ###
from KAIR_light.models.vrt_models import VRT_SR_6_frames_checkpoint, VRT_SR_6_frames_checkpoint_img_sz_160, VRT_denoise_SR
...
# model_denoise_path = '/raid/Pytorch_Checkpoints/2022-08-09/Model_New_Checkpoints/VRT_denoise_general_ranges.py' \
#                 '/VRT_denoise_general_ranges.py_TEST1_Step111975.tar'
...
model_checkpoints = [model_denoise_path, model_sr_path]
...
Train_dict.debug_step = 150
Train_dict.save_models_frequency = 150
Train_dict.validation_set_frequency = 150
Train_dict.tensorboard_train_frequency = 150
...
IO_dict.number_of_images_per_video_to_load = 6
### Miscellenous: ###
IO_dict.flag_normalize_images_to_PPP = True #True
IO_dict.PPP = [5, 10]  # Photons per Pixel.
### Noise Addition: ###
temp_factor = 12/255
IO_dict.noise_dict = get_default_noise_dict()

IO_dict.noise_dict.PRNU_sigmas_polynomial_params = [(3*temp_factor)]
IO_dict.noise_dict.RRNU_sigmas_polynomial_params = [(3*temp_factor)]
IO_dict.noise_dict.CRNU_sigmas_polynomial_params = [(3*temp_factor)]

sigma = 12
IO_dict.noise_dict.per_pixel_readout_noise_sigma = (0, sigma)
IO_dict.noise_dict.row_readout_noise_sigma = (3*temp_factor)
IO_dict.noise_dict.col_readout_noise_sigma = (3*temp_factor)
...
IO_dict.noise_dict.flag_quantize_images = True
IO_dict.noise_dict.flag_add_shot_noise = True
IO_dict.noise_dict.flag_add_blob_NU = False

IO_dict.noises_flags = []
# Model Noises
IO_dict.noise_dict.flag_add_per_pixel_readout_noise = True # 1)
IO_dict.noise_dict.flag_add_row_readout_noise = False # 2)
IO_dict.noise_dict.flag_add_col_readout_noise = False # 3)
IO_dict.noises_flags.extend([IO_dict.noise_dict.flag_add_per_pixel_readout_noise, IO_dict.noise_dict.flag_add_row_readout_noise, IO_dict.noise_dict.flag_add_col_readout_noise])

IO_dict.noise_dict.flag_temporal_gain_per_pixel = False # 4) (speckles)
IO_dict.noises_flags.extend([IO_dict.noise_dict.flag_temporal_gain_per_pixel])

IO_dict.noise_dict.flag_add_per_pixel_NU = False # 5+6)
IO_dict.noise_dict.flag_add_row_NU = False # 7+8)
IO_dict.noise_dict.flag_add_col_NU = False # 9+10)

IO_dict.noises_flags.extend([IO_dict.noise_dict.flag_add_per_pixel_NU] * len(IO_dict.noise_dict.PRNU_sigmas_polynomial_params))
if len(IO_dict.noise_dict.PRNU_sigmas_polynomial_params)==1:
    IO_dict.noises_flags.append(False)
IO_dict.noises_flags.extend([IO_dict.noise_dict.flag_add_row_NU] * len(IO_dict.noise_dict.RRNU_sigmas_polynomial_params))
if len(IO_dict.noise_dict.RRNU_sigmas_polynomial_params)==1:
    IO_dict.noises_flags.append(False)
IO_dict.noises_flags.extend([IO_dict.noise_dict.flag_add_col_NU] * len(IO_dict.noise_dict.CRNU_sigmas_polynomial_params))
if len(IO_dict.noise_dict.CRNU_sigmas_polynomial_params)==1:
    IO_dict.noises_flags.append(False)
...
IO_dict.transforms_dict.downsampling_factor = 4
### Training Flags: ###
...
w,h,offset = 1080,1920,0# 12,512,0
...
IO_dict_test.number_of_image_frames_to_generate = 6
IO_dict_test.number_of_images_per_video_to_load = 6
#######################################################################################################################################
...
# imshow_torch(a[-1]/255)
# imshow_torch(b[-1]/255)
# plt.show(block=True)
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_SR(checkpoints=model_checkpoints, train_devices=train_devices, test_devices=test_devices, pretrained=[True,True])
