...
# model_denoise_path = osp.join(Network_checkpoint_folder, 'Denoise_drones_1VRT_6_Frames_8_GPUs_RGB.py/Denoise_drones_1VRT_6_Frames_8_GPUs_RGB.py_TEST1_Step12300.tar')
# model_denoise_path = osp.join(Network_checkpoint_folder, 'Denoise_drones_1VRT_6_Frames_8_GPUs_RGB.py/Denoise_drones_1VRT_6_Frames_8_GPUs_RGB.py_TEST1_Step12300.tar')
...
model_denoise_path = osp.join(Network_checkpoint_folder, 'Denoise_drones_SBNUC_RGB.py/Denoise_drones_SBNUC_RGB.py_TEST1_Step10425.tar')
...
Train_Images_Folder = osp.join(datasets_main_folder, 'drones/S/train')
...
Test_Images_Folder = osp.join(datasets_main_folder, 'drones/S/test')
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + '_low_light'
...
IO_dict.PPP = 1 #[2, 10]  # Photons per Pixel.
### Noise Addition: ###
IO_dict.noise_dict = get_default_noise_dict()
IO_dict.noise_dict.RRNU_sigmas_polynomial_params = [0.5,0.5]
IO_dict.noise_dict.CRNU_sigmas_polynomial_params = [0.5,0.5]
IO_dict.BRNU_sigmas_polynomial_params = [1, 1, 0]
IO_dict.PRNU_sigmas_polynomial_params = [1, 0.05, 0.00001]
...
IO_dict.noise_dict.per_pixel_readout_noise_sigma = 2
### Post Loading Stuff: ###
...
######################################################################################################################################
### Show images: ###
# a = train_dataset[0].output_frames_original
# imshow_torch(a[0]/33)
# plt.show(block=True)
#####################################################################################################################################################################
### Model Itself: ###
