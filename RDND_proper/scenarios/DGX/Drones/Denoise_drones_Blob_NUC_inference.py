...
### Import Models: ###
from KAIR_light.models.vrt_models import VRT_denoise_6_frames_8_GPUs
### Initialize Network State Dict: ###
Train_dict = EasyDict()
######################################################################################################################
### Paths: ###
#(1). General Paths:
home_path = '/home/davidk' #Path.home()
...
model_denoise_path = osp.join(Network_checkpoint_folder, 'Denoise_drones_Blob_NUC.py/Denoise_drones_Blob_NUC.py_TEST1_Step675.tar')
...
Train_dict.Network_checkpoint_prefix = os.path.basename(__file__) + '1000 frames'
...
print ('Starting to train on GPUs: {}'.format(devices))
...
IO_dict.noise_dict.BRNU_sigmas_polynomial_params = [3]  # Blob Readout Non Uniformity. polynomial coefficients
IO_dict.noise_dict.blob_NU_blob_size = 10
IO_dict.noise_dict.flag_same_blob_NU_on_all_channels = True
IO_dict.noise_dict.flag_same_blob_NU_on_all_batch = False
...
IO_dict.noise_dict.per_pixel_readout_noise_sigma = 0
### Post Loading Stuff: ###
...
IO_dict_test.number_of_image_frames_to_generate = 300
IO_dict_test.number_of_images_per_video_to_load = 300
#######################################################################################################################################
...
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames_8_GPUs(False, model_denoise_path, IO_dict.model_devices)
...
inference_path = os.path.join(inference_path_master, 'NewInference', Train_dict.Network_checkpoint_prefix + '/' + dataset_name)
...
#
# def save():
#     os.system("ffmpeg -r 1 -i 0000_everything_concat_Frame%001d.png -vcodec mpeg4 -y movie.mp4")
#
# ffmpeg -r 1 -i 0000_everything_concat_Frame%001d.png -vcodec mpeg4 -y movie.mp4
#
#
# import os
# import moviepy.video.io.ImageSequenceClip
# image_folder='../Pytorch_Checkpoints/New Inference/Denoise_drones_Blob_NUC_inference.pyInference only/RapidBase__TrainingCore__datasets__DataSet_Videos_In_Folders_LoadCorruptedGT/save_folder__validation/0000/0000/Concatenated_Outputs/output.mp4'
# fps=1
#
...
# (
#     ffmpeg
#     .input(image_folder+ '/*.jpg', pattern_type='glob', framerate=25)
#     .output('movie2.mp4')
#     .run()
# )
#
...
#
#
# ffmpeg -framerate 25 -i 0000_everything_concat_Frame-%000d.png -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p output.mp4
#
#
