...
model_deblur_path = '/raid/Pytorch_Checkpoints/2022-05-31/Model_New_Checkpoints/drones_RGB_WN.py_2/drones_RGB_WN.py_2_TEST1_Step7575.tar'
...
#(2). Train Images:
# Train_Images_Folder = osp.join(datasets_main_folder, 'REDS')
...
Test_Images_Folder = osp.join(datasets_main_folder, 'REDS')
# Test_Images_Folder = osp.join(datasets_main_folder, 'REDS')# Test_Images_Folder = os.path.join(datasets_main_folder, 'Example Videos/Beirut_BW_Original')
...
IO_dict.Corrupted_string_pattern_to_search = '*val_sharp*'
...
IO_dict_test.number_of_image_frames_to_generate = 100
IO_dict_test.number_of_images_per_video_to_load = 100
#######################################################################################################################################
...
def save_dataloader(dataloader, num_imgs=20, num_frames=50, val=False, data_dir = '../Data4', dataset_name = 'dataset'):
...
save_dataloader(test_dataloader,30,100,data_dir = '../REDS_PP=5')
...
######################################################################################################################################
### Show images: ###
c = test_dataset[5].output_frames_noisy
b = test_dataset[5].output_frames_original

for a in test_dataloader:
    a=a
    break
b = a['output_frames_noisy'][0][0]
c = a['output_frames_original'][0][0]
# a = batch_noisy[0][0]
imshow_torch(c/c.max())
imshow_torch(b/b.max())
imshow_torch(img/img.max())
plt.show(block=True)
#####################################################################################################################################################################
### Model Itself: ###
model = VRT_denoise_6_frames_checkpoint(pretrained=True, checkpoint_path=model_denoise_path,
                train_device=train_devices[0], test_device=test_devices[0], test_args=IO_dict.test_args)
