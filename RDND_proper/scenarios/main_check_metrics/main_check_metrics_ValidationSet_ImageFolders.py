...
running_mean_images_path = path_fix_path_for_linux('/home/mafat/Pytorch_Checkpoints/Inference/model_combined/DataSet_Videos_In_Folders_AddNoise/Div2k_validation_set_BW_video_TESTING')
###
...
clean_images, clean_image_filenames = read_images_and_filenames_from_folder(original_images_path,
                                      ...
                                      flag_to_BW=False,
                                      flag_random_first_frame=False)
running_mean_images, running_mean_image_filenames = read_images_and_filenames_from_folder(running_mean_images_path,
                                      flag_recursive=True,
                                      crop_size=np.inf,
                                      max_number_of_images=np.inf,
                                      allowed_extentions=IMG_EXTENSIONS,
...
center_frame_running_mean_filenames_and_indices = [(index,x) for index,x in enumerate(running_mean_image_filenames) if 'running_mean' in x]
...
for i in np.arange(len(center_frame_clean_image_filenames_and_indices)):
    ...
    center_frame_running_mean_index, center_frame_running_mean_filename = center_frame_running_mean_filenames_and_indices[i]
    ...
    center_frame_running_mean_images.append(running_mean_images[center_frame_running_mean_index])
######################################################################################################################################
...
for index in np.arange(len(center_frame_noisy_images)):
    ...
    center_frame_running_mean_torch = torch.Tensor(center_frame_running_mean_images[index]).permute([2,0,1]).unsqueeze(0)
    ...
    inputs_dict['center_frame_pseudo_running_mean'] = center_frame_running_mean_torch/255
