...
results_path_name_addition = 'KAYA23_Avg'
...
for i in np.arange(image_index_to_start_from, len(original_frames_dataset.image_filenames_list_of_lists[0])):
    ...
    quantile_list.append(current_image.quantile(0.99))
images_list = torch.cat(images_list,0)
max_to_use = np.quantile(quantile_list,0.5)
stretch_factor = 255/max_to_use/1.2
# imshow_torch(images_list[0].clamp(0,max_to_use))
# imshow_torch((images_list[0]*stretch_factor).clamp(0,255))

### Get Noise Profile Using Histogram Algorithm: ###
histogram_spatial_binning_factor = 3
crude_histogram_bins_vec = my_linspace(0, 0.25, 10)
downsample_kernel_size = 1
noisy_frames_current = images_list / 255
clean_frames_estimate = noisy_frames_current.median(0)[0].unsqueeze(0)
noise_map_estimate_peak, SNR_estimate_peak, noise_as_function_of_gray_level, histograms_bins_list, histograms_list  = \
...
for frame_index in np.arange(image_index_to_start_from, len(original_frames_dataset.image_filenames_list_of_lists[0])):
    print(frame_index)
    ### Get Next Frame: ###
    ...
    ### Combine Images: ###
    clean_frame_current, pixel_counts_map_current = combine_frames(noisy_frame_current_cropped.data,
                                         clean_frame_previous_warped.data,
    ...
    ### Save Wanted Images: ###
    save_image_numpy(save_path1, str(frame_index) + '_center_frame_original.png', (original_frame_current_cropped[0] * 255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0), False, flag_scale=False)
    save_image_numpy(save_path2, str(frame_index) + '_clean_frame_estimate.png', (clean_frame_current[0] * 255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0), False, flag_scale=False)
    save_image_numpy(save_path3, str(frame_index) + '_center_frame_noisy.png', (noisy_frame_current_cropped[0]* 255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0), False, flag_scale=False)
    ...
    image_to_write = (image_to_write*255*stretch_factor).clip(0,255).astype(np.uint8)
    ...
    title('Inferred Signal Mean: ' + str((float(decimal_notation((clean_frame_current*255).mean(),1)))) + ', ' +
          'Inferred Sigma: ' + str((float(decimal_notation((noise_map_current*255).mean(),1)))) + ', ' +
    ...
    clean_frame_current_numpy = ((clean_frame_current[0]*255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0)).astype(uint8)
    noisy_frame_current_cropped_numpy = ((noisy_frame_current_cropped[0]*255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0)).astype(uint8)
    original_frame_current_cropped_numpy = ((original_frame_current_cropped[0]*255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0)).astype(uint8)
    perfect_average_frame_current_cropped_numpy = ((clean_frame_perfect_averaging[0]*255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0)).astype(uint8)
    simple_average_frame_current_cropped_numpy = ((clean_frame_simple_averaging[0]*255*stretch_factor).clamp(0,255).cpu().numpy().transpose(1, 2, 0)).astype(uint8)
    original_frame_seed_cropped_numpy = ((original_frame_seed_cropped[0]*255*stretch_factor).clamp(0, 255).cpu().numpy().transpose(1, 2, 0) * 255).astype(uint8)
