...
from _alignments.cross_correlation_alignments import format_parameters_classic_circular_cc
from _internal_utils.test_exceptions import raise_if_not, raise_if
from RapidBase.import_all import *
from _alignments.minSAD_alignments import interpolated_minSAD_defaults, format_min_SAD_params, \
    parabola_fit_1d_over_n_dimensions, format_minSAD_align_parameters
from _internal_utils.torch_utils import unravel_index_torch, NoMemoryMatrixOrigami, construct_tensor, \
    true_dimensionality, multiplicable_tensors
import kornia as K
import warnings

from _transforms.affine_transformations import affine_transform_interpolated
...
class Gimbaless_Homography_Layer_Torch(nn.Module):
    ...
    def __init__(self):
        ...
        self.dict_from_matlab = scipy.io.loadmat(
            r'C:\Users\dudyk\Desktop\dudy_karl\RDND\RapidBase\Data/spatial_lowpass_9tap.mat')  # Read this from file
    ...
    def forward(self, input_tensor, reference_tensor=None):
        ...
        if self.X is None:
            derivative_filter_lowpass_equivalent_length = len(
                self.spatial_lowpass_before_temporal_derivative_filter_x)
            shift_filter_length = 11  # TODO: later on with the full version there will be an equivalent shift filter
            invalid_frame_size = np.round((derivative_filter_lowpass_equivalent_length + shift_filter_length) / 4)
            x_start = -input_tensor.shape[-1] / 2 + 0.5 + invalid_frame_size
            x_stop = input_tensor.shape[-1] / 2 - 0.5 - invalid_frame_size
        ...
        # py = self.torch_convn_layer.forward(input_tensor, self.grad_y_kernel.flatten(), -2)
        # (2). input_tensor + reference_tensor
        I_x = self.torch_convn_layer.forward((input_tensor + reference_tensor) / 2, self.grad_x_kernel.flatten(),
                                             -1)
        I_y = self.torch_convn_layer.forward((input_tensor + reference_tensor) / 2, self.grad_y_kernel.flatten(),
                                             -2)
        ...
        ######################################################################################################
        ### Calculate Temporal Derivative: ###
        I_t_xconv = self.torch_convn_layer.forward(input_tensor - reference_tensor,
                                                   self.spatial_lowpass_before_temporal_derivative_filter_x.flatten(),
                                                   -1)
        I_t_yconv = self.torch_convn_layer.forward(input_tensor - reference_tensor,
                                                   self.spatial_lowpass_before_temporal_derivative_filter_y.flatten(),
                                                   -2)
        ...
        p = torch.cat([I_x.unsqueeze(-1), I_y.unsqueeze(-1), M1_operator.unsqueeze(-1), M2_operator.unsuqeeze(-1),
        ...
        d2 = torch.cat([(I_t_xconv * I_x).sum(-1, True),
                        ...
                        (I_t_xconv * (-I_x * self.X_flattened * 2) + I_t_yconv(
                            -I_y * self.X_flattened * self.Y_flattened)).sum(-1, True),
                        (I_t_xconv * (-I_x * self.X_flattened * self.Y_flattened) + I_t_yconv * (
                                    -I_y * self.Y_flattened ** 2)).sum(-1, True)], -1).squeeze(0).unsqueeze(-1)
        d_vec = torch.matmul(d1, d2).squeeze(-1)
        delta_x = d_vec[:, 0]
        delta_y = d_vec[:, 1]
        delta_theta = d_vec[:, 2]
    ...
    def forward_weighted(self, input_tensor, reference_tensor=None, W=None):
        ...
        d2 = torch.cat([(W_flattened * I_t_xconv * I_x).sum(-1, True),
                        ...
                        (W_flattened * I_t_xconv * (I_x * self.Y_flattened) - W_flattened * I_t_yconv * (
                                    I_y * self.X_flattened)).sum(-1, True),
                        (W_flattened * I_t_xconv * (-I_x * self.X_flattened * 2) + W_flattened * I_t_yconv(
                            -I_y * self.X_flattened * self.Y_flattened)).sum(-1, True),
                        (W_flattened * I_t_xconv * (
                                    -I_x * self.X_flattened * self.Y_flattened) + W_flattened * I_t_yconv * (
                                     -I_y * self.Y_flattened ** 2)).sum(-1, True)], -1).squeeze(0).unsqueeze(-1)
        d_vec = torch.matmul(d1, d2).squeeze(-1)
        delta_x = d_vec[:, 0]
        delta_y = d_vec[:, 1]
