...
class GenerateSegmentIndices:
    ...
    def __init__(self, interval_list, start_idx=0, test=False, filename_tmpl='{:08d}.png'):
        self.interval_list = interval_list
        self.filename_tmpl = filename_tmpl
    ...
    def __call__(self, results):
        ...
        if self.sequence_length - num_input_frames * interval < 0:
            raise ValueError('The input sequence is not long enough to '
                             ...
                             '[num_input_frames]. {}-{}-{}'.format(self.sequence_length, num_input_frames, interval))
        if self.test:
            start_frame_idx = self.start_idx
