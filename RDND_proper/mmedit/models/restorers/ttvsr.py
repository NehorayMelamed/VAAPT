...
class TTVSR(BasicRestorer):
    ...
    def __init__(self,
        ...
        self.freeze_dct = freeze_dct

        self.fix_ttvsr = train_cfg.get('fix_ttvsr', 0) if train_cfg else 0
    ...
    def train_step(self, data_batch, optimizer):
        ...
        if self.step_counter < self.fix_iter:
            ...
            if self.freeze_dct:
                for k,v in self.generator.named_parameters():
                    if 'dct' in k or 'rdct' in k:
                        v.requires_grad_(False)
        
        if self.step_counter < self.fix_ttvsr:
            for k,v in self.generator.named_parameters():
                if 'FTT' not in k:
                    v.requires_grad_(False)
        elif self.step_counter == self.fix_ttvsr:
            self.generator.requires_grad_(True)
            if self.freeze_dct:
                for k,v in self.generator.named_parameters():
                    if 'dct' in k or 'rdct' in k:
    ...
    def forward_test(self,
        ...
        if gt is not None and gt.ndim == 4:
            ...
            if self.check_if_mirror_extended(lq):  # with mirror extension
                output = 0.25 * (output[:, t // 8] + output[:, t // 8 + 7] + output[:, -1 - t // 8 - 7] + output[:, -1 - t // 8])
