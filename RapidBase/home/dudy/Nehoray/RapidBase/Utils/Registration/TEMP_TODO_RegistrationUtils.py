...
def register_images(img1, img2,
    ...
    if registration_algorithm == 'MAOR':
        ...
        shifts_array, z_max, clean_frame_previous_warped = register_images_CrossCorrelation(img1, img2, search_area,
                                                                                            inner_crop_size_to_use)
        flow_map = None
    elif registration_algorithm == 'NormalizedCrossCorrelation':
        shifts_array, z_max, clean_frame_previous_warped = register_images_NCC(img1, img2, search_area,
                                                                               inner_crop_size_to_use,
                                                                               downsample_factor)
        flow_map = None
    elif registration_algorithm == 'PhaseCorrelation':
        shifts_array, z_max, clean_frame_previous_warped = register_images_PhaseCorrelation(img1, img2, search_area,
                                                                                            inner_crop_size_to_use)
        flow_map = None
        ...
        shifts_array, clean_frame_previous_warped, flow_map = register_images_OpticalFlow(img1, img2,
                                                                                          'DeepFlow')  # DeepFlow, SimpleFlow, and TVL1
...
def calculate_SAD_torch(tensor1, tensor2, search_area=5):
    ...
    for iin in arange(len(DispUD)):
        for jjn in arange(len(DispLR)):
            ...
            SAD[:, :, iin, jjn] = (Broi - Atmp).abs().sum(2, False).sum(2,
                                                                        False)  # SAD, maybe this can be better refined but still
...
def shift_image_torch(img, dx, dy):
    ...
    N, M = img.size()
    # fft needs the last dim to be 2 (real,complex) TODO: faster implementation
    img_padded = torch.stack((img, torch.zeros(N, M)),
                             dim=2)  # make into a pseudo complex number with the imaginary part being zero
    ...
    X = torch.from_numpy(
        tmp.view("(2,)float")).float()  # Make tmp into a pseudo complex number with two channels (real,img)
