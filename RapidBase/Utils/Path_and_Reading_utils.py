...
import cv2
import torch
# from RapidBase.Utils.Array_Tensor_Manipulation import *
...
# Default Image: #
# from RapidBase.Utils.Pytorch_Numpy_Utils import get_full_shape_torch

default_image_filename_to_load1 = '/raid/datasets/DIV2K/DIV2K/Flickr2K/000001.png'
default_image_filename_to_load2 = '/raid/datasets/DIV2K/DIV2K/Flickr2K/000002.png'
default_image_filename_to_load3 = '/raid/datasets/DIV2K/DIV2K/Flickr2K/000003.png'

default_video_path_to_load1 = '/raid/datasets/binning_datasets/VisDrone/sequences/00001'
default_video_path_to_load2 = '/raid/datasets/binning_datasets/VisDrone/sequences/00002'
default_video_path_to_load3 = '/raid/datasets/binning_datasets/VisDrone/sequences/00003'
...
def read_video_default_torch(video_dir = None, size=(128,128), video_num=1, flag_convert_to_rgb=1):
    # Add the same with a video
    video = []
    if video_dir is None:
        if video_num ==1:
            video_dir = default_video_path_to_load1
        if video_num == 2:
            video_dir = default_video_path_to_load2
        if video_num == 3:
            video_dir = default_video_path_to_load3
    video_images = sorted(os.listdir(video_dir))
    for img in video_images:
        image = read_image_torch(os.path.join(video_dir,img),flag_convert_to_rgb)/255
        transform = transforms.CenterCrop(size)
        image_crop = transform(image)
        video.append(image_crop)
    video = (torch.stack(video)).squeeze(1)
    return video
...
def save_image_numpy(folder_path=None,filename=None,numpy_array=None,flag_convert_bgr2rgb=True, flag_scale=True, flag_save_uint16=False, flag_convert_to_uint8=False):
    ...
    if flag_save_uint16:
        ...
        imageio.imwrite(os.path.join(folder_path, filename), array_to_save.astype(np.uint16))
    else:
...
def read_images_from_filenames_list(image_filenames, flag_return_numpy_or_list='numpy', crop_size=100, max_number_of_images=10, allowed_extentions=IMG_EXTENSIONS, flag_how_to_concat='C', crop_style='random', flag_return_torch=False, transform=None, flag_to_BW=False, flag_random_first_frame=False, first_frame_index=-1, flag_to_RGB=False, flag_return_position_indices=False, start_H=-1, start_W=-1):
    ...
    for current_index in np.arange(start_index, stop_index, 1):
        ...
        if is_image_file(image_filename, allowed_extentions):
            ...
            if (current_image.shape[0] >= crop_size[0] and current_image.shape[1] >= crop_size[1]) or (crop_size == np.inf or crop_size == [np.inf, np.inf]):
                ...
                current_crop = crop_numpy(current_image, crop_size_x=crop_size[0], crop_size_y=crop_size[1], crop_style=crop_style, start_H=start_H, start_W=start_W)
...
def video_images_to_video(images_path, video_name='my_movie.avi'):
    ...
    video_writer = cv2.VideoWriter(video_full_filename, fourcc, 25.0, (W, H))
...
def convert_inference_folder_to_video(inference_images_path):
    ### Get images in numerical order: ###
    inference_images_path = 'G:\GITLAB\hla-deep-learning\depth_post_processing\Checkpoints\Inference\Magic_Converge_Passthrough_Gibson\Video_Dataset\Inference'
    inference_images_path = 'G:\GITLAB\hla-deep-learning\depth_post_processing\Checkpoints\Inference\Magic_Converge_CSPN9Ch_ConfHole_Check_3\Video_Dataset\Inference'
    # files_list_conf_out = path_get_files_recursively(inference_images_path, 'input_left_disparity_maksed_confidence_out_55')
    files_list_conf_out = path_get_files_recursively(inference_images_path, 'output_left_disparity_maksed_confidence_out_55')
    files_list_conf_in = path_get_files_recursively(inference_images_path, 'input_left_disparity_maksed_confidence_in')
    files_list_image_in = path_get_files_recursively(inference_images_path, 'input_left_image')
    image_number_list = []
    for file in files_list_image_in:
        image_number_list.append(int(os.path.split(file)[1].split('_')[0]))
    sorted_indices_list = np.argsort(image_number_list)

    ### Get sample image: ###
    image = cv2.imread(files_list_image_in[0], cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    height, width, channels = image.shape

    ### Create video: ###
    video_name = os.path.join(inference_images_path,'video_out.avi')
    video_object = cv2.VideoWriter(video_name, 0, 25, (width*3,height*1))
    for index in sorted_indices_list:
        # print(files_list[index])
        current_image = cv2.imread(files_list_image_in[index])
        current_conf_in = cv2.imread(files_list_conf_in[index])
        current_conf_out = cv2.imread(files_list_conf_out[index])
        final_image = np.concatenate((current_image,current_conf_in,current_conf_out),1)
        video_object.write(final_image)

    cv2.destroyAllWindows()
    video_object.release()

def change_filenames_rjust():
    filenames_list = path_get_all_filenames_from_folder('/home/mafat/DataSets/pngsh/pngs/6')
