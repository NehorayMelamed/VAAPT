...
def ECC_torch(input_tensor, reference_tensor, number_of_levels, number_of_iterations_per_level, transform_string, delta_p_init=None, flag_print=False):
...
class ECC_Layer_Torch(nn.Module):
    ...
    def spatial_interpolation_torch(self, input_image, H_matrix, interpolation_method, transform_string, x_vec, y_vec, X_mat, Y_mat, H, W):
        ...
        out = torch.nn.functional.grid_sample(input_image.unsqueeze(0).unsqueeze(0), bilinear_grid, mode='bicubic').squeeze(0).squeeze(0)  #TODO: get rid of the unsqueeze by being consistent with the format
        #################################################################################################################################
    ...
    def image_jacobian_torch(self, gx, gy, J_list, number_of_parameters):
        ...
        # # bla_1 = torch.cat([gx*Jx, gx*J0, -gx*Jxx_prime, gx*Jy, gx*J0, -gx*Jyx_prime, gx*J1, gx*J0], -1)
        # # bla_2 = torch.cat([gy*J0, gy*Jx, -gy*Jxy_prime, gy*J0, gy*Jy, -gy*Jyy_prime, gy*J0, gy*J1], -1)
        # # bla_3 = bla_1 + bla_2
        # bla_3 = torch.cat([gx * Jx + gy * J0, gx * J0 + gy * Jx, -gx * Jxx_prime - gy * Jxy_prime, gx * Jy + gy * J0, gx * J0 + gy * Jy, -gx * Jyx_prime - gy * Jyy_prime, gx * J1 + gy * J0, gx * J0 + gy * J1], -1)
        # # bla_3 - G
        # H, W = J0.shape[-2:]
        # bla_4 = torch_reshape_image(gx * Jx + gy * J0, (H * W, 1), order='F').contiguous()
        # bla_4 = torch_reshape_image(gx * J0 + gy * Jx, (H * W, 1), order='F').contiguous()

        ### Unroll All Variables: ###
        [Jx, Jy, J0, J1, Jxx_prime, Jxy_prime, Jyx_prime, Jyy_prime] = J_list
        G0 = gx * Jx + gy * J0
        G1 = gx * J0 + gy * Jx
    ...
    def forward(self, input_tensor, reference_tensor, delta_p_threshold=2e-3):
        ...
        [vy, vx] = torch.gradient(input_tensor_output_list[0], dim=[-1, -2])
        ...
        for level_index in np.arange(self.number_of_levels, 0, -1):  # start with lowest resolution (highest level of the pyramid)
            ...
            for iteration_index in np.arange(self.number_of_iterations_per_level):
                ...
                #TODO: again, understand how to make this faster!!!.
                #TODO: besides, many things here have a M_transposed @ Other_Vec thing going on! understand how can i make this faster!!!!
                ### Compute projections of images into G: ###
                #(*). Calculate Gt:
        ...
        C_input = 1
        warpedImage = torch.zeros_like(input_tensor)
        warpedImage[:, :] = spatial_interpolation_torch(input_tensor[:, :], final_warp, 'linear', self.transform_string, nx2, ny2, H, W)
        # for ii in torch.arange(C_input):
...
def ECC_demo_numpy():
    ...
    # reference_tensor = read_image_torch(path1)
    # input_tensor = read_image_torch(path2)
    # input_tensor = RGB2BW(input_tensor[0]).cuda()[0]/255
    # reference_tensor = RGB2BW(reference_tensor[0]).cuda()[0]/255
    # ECC_layer_object = ECC_Layer_Torch(reference_tensor, number_of_iterations_per_level=500, number_of_levels=1, transform_string='homography', delta_p_init=delta_p_init)
    # H_matrix, warpedImage = ECC_layer_object.forward(input_tensor, reference_tensor)
    ...
    # reference_tensor = read_image_torch(path1)
    # input_tensor = read_image_torch(path2)
    # H_matrix_initial = torch.tensor(delta_p_init).to(input_tensor.device)
    # tic()
    # aligned_tensor_CC_ECC, H_matrix_OpenCV, shifts_rotation_CC_ECC, _, shifts_h_CC_ECC, shifts_w_CC_ECC = \
    #     Align_ECC(input_tensor,
    #               reference_tensor,
    #               warp_mode='homography',
    #               warp_matrix=H_matrix_initial,
    #               iterations=100)
    # toc()
    # imshow_torch(RGB2BW(input_tensor))
    # imshow_torch(RGB2BW(reference_tensor))
    # imshow_torch(aligned_tensor_CC_ECC.squeeze(0))
    ...
    H_matrix, warpedImage = ECC_torch(reference_tensor,
                                      input_tensor,
                                      number_of_levels=1,
                                      number_of_iterations_per_level=100,
